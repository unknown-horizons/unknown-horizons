...
def get_fife_path(fife_custom_path=None):
	...
	for p in _paths:
		if p in sys.path:
			# skip dirs where import would have found FIFE
			continue
		p = os.path.abspath(p)
...
def is_fife_path(path):
	"""Checks if typical FIFE directories exist in path.
	This does not guarantee that it really is a FIFE dir, but usually works."""
...
def restart_with_fife(fife_custom_path=None):
	# FIXME: fife was never written into LD_LIBRARY_PATH, but old comment said so
...
def _update_environment_variable(name, iterable, pathsep=os.pathsep):
	"""Updates environment variable `name` with the contents of `iterable`
	using `pathsep` as separator.
	"""
	env_var = list(os.environ.get(name, []))
	os.environ[name] = pathsep.join(itertools.chain(iterable, env_var))
