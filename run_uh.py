...
def main():
    ...
    try:
        ...
        # Workaround for "locale.Error: unsupported locale setting"
        pass
    ...
    if not options.profile:
        ...
        pattern = os.path.join(profiling_dir,
                               time.strftime('%Y-%m-%d') + '.%02d.prof')
        num = 1
...
def setup_debugging(options):
    ...
    # not too nice way of sharing code, but it is necessary because code
    # from this file can't be accessed elsewhere on every distribution,
    #  and we can't just access other code.
    ...
    if options.debug or options.debug_module or options.debug_log_only:
        ...
        # log any other stdout output there (this happens,
        #  when FIFE c++ code launches some
        # FIFE python code and an exception happens there).
        #  The exceptionhook only gets
        ...
        # without getting logs twice + without enabling debug log
        # for everything (see first if-clause inside that method)
    ...
    try:
        ...
        try:
            ...
            if str(e) != 'cannot import name fife':
                ...
                log().warning(str(e))
                if str(e) == 'DLL load failed: %1 is not a valid' \
                             ' Win32 application.':
                    # We found FIFE but the Python and FIFE architectures
                    # don't match (Windows).
                    exit_with_error('Unsupported Python version',
                                    '32 bit FIFE requires 32 bit (x86)'
                                    ' Python 2.')
            return False
    except ImportError:
...
def find_fife():
    ...
    if options.fife_path:
        ...
        # Support giving the path to
        # FIFE_ROOT/engine/python/fife/__init__.pyc etc.
    ...
    for opt1 in ('.', '..', '..' + os.sep + '..'):
        for opt2 in ('.', 'fife', 'FIFE', 'Fife', 'fifengine'):
            for opt3 in ('.', 'trunk'):
                path = os.path.abspath(os.path.join('.', opt1, opt2, opt3,
                                                    'engine', 'python'))
...
def setup_fife():
    ...
    if (fife_version_major, fife_version_minor,
        ...
        log().warning(
            'Unsupported fife version %s.%s.%s,'
        ...
        log().debug(
            'Using fife version %s.%s.%s, at least %d.%d.%d required',
