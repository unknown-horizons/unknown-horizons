...
class TabWidget(object):
	...
	def __init__(self, system_id, object=None, callbacks={}):
		self.object = object
		self.tabs = []
		for name, xml, up, down, hover in game.main.db("SELECT name, xml, button_up_image, button_down_image, button_hover_image FROM tabs WHERE rowid IN (SELECT tab_id FROM tab_system WHERE system_id=? ORDER BY position)", system_id):
		...
		self.widget.position = (
			game.main.session.ingame_gui.gui['minimap'].position[0] - self.widget.size[0] - 5 if game.main.fife.settings.getScreenWidth()/2 + self.widget.size[0]/2 > game.main.session.ingame_gui.gui['minimap'].position[0] else game.main.fife.settings.getScreenWidth()/2 - self.widget.size[0]/2,
			game.main.fife.settings.getScreenHeight() - self.widget.size[1] - 20
		)
