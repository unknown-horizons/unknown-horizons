...
class Tear(object):
	...
	def __call__(self, issuer):
		...
		@param issuer: the issuer of the command
		"""
		(game.main.session.world.islands[self.island_id] if self.settlement_id is None else game.main.session.world.islands[self.island_id].settlements[self.settlement_id]).buildings[self.building_id].deconstruct()
