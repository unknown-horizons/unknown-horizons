...
def setup_package():
    """
    Setup read-only database. This might have to change in the future, tests
    should not fail only because a production
    now takes 1 second more in the game.
    """
    global db
...
def _dbreader_convert_dummy_objects():
    ...
    We fix this by replacing it with zero, SQLite doesn't care much about
    types, and hopefully a number is less likely to break code than None.
...
class SPTestSession(SPSession):
    ...
    def __init__(self, rng_seed=None):
        ...
        super(SPTestSession, self).__init__(horizons.globals.db, rng_seed,
    ...
    def save(self, *args, **kwargs):
        ...
        """
        # SavegameManager._write_screenshot tries to create a screenshot and
        # breaks when accessing fife properties
        with mock.patch('horizons.session.SavegameManager._write_screenshot'):
            # We need to covert Dummy() objects to a sensible value that can
    ...
    def cleanup(cls):
        """
        If a test uses manual session management, we cannot be sure that
        session.end was called before a crash, leaving the game in an unclean
        state. This method should return the game to a valid state.
        """
    ...
    def run(self, ticks=1, seconds=None):
        """
        Run the scheduler the given count of ticks or (in-game) seconds.
        Default is 1 tick, if seconds are passed, they will overwrite
        the tick count.
        """
        if seconds:
...
def new_session(mapgen=create_map, rng_seed=RANDOM_SEED, human_player=True,
                ai_players=0):
    """
    Create a new session with a map, add one human player and a trader
    (it will crash otherwise). It returns both session and player to avoid
    making the function-based tests too verbose.
    """
...
def game_test(timeout=15*60, mapgen=create_map, human_player=True,
    ...
    function based tests can't pass arguments to the test, or keep a reference
    somewhere.
    If a test fails, we need to reset the session under all circumstances,
    otherwise we break the rest of the tests. The global database reference
    has to be set each time too, unittests use this too, and we can't predict
    the order tests run (we should not rely on it anyway).
    """
        ...
        def wrapped(*args):
            ...
            try:
                ...
                try:
                    ...
                    # This is ok since cleanup is only defined to work
                    # when invariants are in place,
