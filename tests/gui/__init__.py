...
def setup_package():
    ...
    as user directory (settings, savegames etc.)
...
class GuiTestPlugin(Plugin):
    ...
    Because nose runs in a different process than the real test,
    we cannot easily show the traceback as if the exception occured here.
    The real traceback will
...
class TestRunner(object):
    ...
    Tests have to be generators. With generators, we can give control back to
    the engine anytime we want and easily continue at that point later.
    ...
    separate call to the engine's mainloop, in turn,
    this would call the `_tick`
    method and attempt to continue the generator (which is still running).

    Therefore, a new generator has to be installed to handle dialogs.
    Handlers are hold in list used as stack - only the last added handler
    will be continued until it has finished.
    """
    __test__ = False
    ...
    def _custom_setup(self):
        ...
        BuildTab.default_build_menu_config = BuildTab.cur_build_menu_config\
            = BuildTab.build_menu_config_per_tier
...
def gui_test(use_dev_map=False, use_fixture=None, ai_players=0,
             timeout=15 * 60, cleanup_userdir=False,
    ...
    def deco(func):
        ...
        def wrapped():
            ...
            args = executable + ['run_uh.py', '--sp-seed', str(RANDOM_SEED),
                                 '--gui-test', test_name]
            if use_fixture:
            ...
            try:
                # if nose does not capture stdout,
                # then most likely someone wants to
                # use a debugger (he passed -s at the cmdline).
                # In that case, we will redirect stdout/stderr
                # from the gui-test process to the testrunner process.
                sys.stdout.fileno()
                stdout = sys.stdout
                stderr = sys.stderr
                nose_captured = False
            except AttributeError:
                # if nose captures stdout, we can't redirect to sys.stdout,
                # as that was replaced by StringIO.
                # Instead we capture it and return the data at the
                # end.
                stdout = subprocess.PIPE
                stderr = subprocess.PIPE
                nose_captured = True

            # Activate fail-fast, this way the game will stop running
            ...
            if proc.returncode != 0:
                if nose_captured:
                    ...
                    if 'Traceback' not in stderr:
                        stderr += '\nNo usable error output received,' \
                                  ' possibly a segfault.'
                    raise TestFailed('\n\n' + stderr.decode('ascii', 'ignore'))
                else:
                    raise TestFailed()

        # we need to store the original function, otherwise the new process
        # will execute this decorator, thus spawning a new process..
...
# FIXME GUI tests still don't work in parallel,
# this is needed for game/unit tests to work
