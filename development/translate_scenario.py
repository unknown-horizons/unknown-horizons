...
YAML_PATH = 'content/scenarios/{scenario}_{language}.yaml'
...
def setup_paths():
	...
	# name of or path to English .yaml scenario file
	scenario_path = sys.argv[1]

	# Mandatory second argument:
	# either path to a specific .po translation catalog for this scenario
	# or ISO 639-1 code of desired translation on pootle (`af`, `hr`, ...)
	language_path = sys.argv[2]

	if not os.path.exists(scenario_path):
		scenario_path = YAML_PATH.format(scenario=scenario_path, language='en')
	if not os.path.exists(scenario_path):
		print 'Scenario file not found:', scenario_path
		sys.exit(1)

	# drop [_en].yaml suffix and paths to file to obtain pootle name
...
def write_translated_yaml(fileish, where, metadata=None):
	...
	Uses a regular gettext .po file to read translations from. This file
	currently is generated by invoking `create_scenario_pot.sh` with the
	scenario name as parameter (same as for this script).
	It is then uploaded to pootle as project `scenarios`.
	"""
...
# using translation files from pootle and the original scenario file
# {1}
\n'''.format(__file__.lstrip('./'), metadata['original']))
