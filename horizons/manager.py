...
class MPManager(LivingObject):
	...
	def hash_value_check(self, tick):
		if tick % self.HASH_EVAL_DISTANCE == 0:
			if not self.checkuphashmanager.are_checkup_hash_values_equal(tick, self.hash_value_diff):
				self.log.error("MPManager: Hash values generated in tick {0!s} are not equal".format(str(tick - self.HASHDELAY)))
	...
	def hash_value_diff(self, player1, hash1, player2, hash2):
		...
		self.log.error("MPManager: Hash diff:\n{0!s} hash1: {1!s}\n{2!s} hash2: {3!s}".format(player1, hash1, player2, hash2))
		self.log.error("------------------")
		...
		for pkg in pkges[1:]:
			if pkges[0].checkup_hash != pkg.checkup_hash:
				if cb_diff is not None:
					...
					cb_diff("local" if pkges[0].player_id==localplayerid else "pl#{0:02d}".format((pkges[0].player_id)),
						pkges[0].checkup_hash,
						"local" if pkg.player_id==localplayerid else "pl#{0:02d}".format((pkg.player_id)),
						pkg.checkup_hash)
				return False
		return True
