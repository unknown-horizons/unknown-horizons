	...
	def _receive_event(self, timeout=SERVER_TIMEOUT):
		...
		try:
			...
			if event.type == enet.EVENT_TYPE_NONE:
				...
				self._reset()
				self.log.warning("Unexpected disconnect from {0!s}".format((event.peer.address)))
				raise network.CommandError("Unexpected disconnect from {0!s}".format((event.peer.address)))
			elif event.type == enet.EVENT_TYPE_CONNECT:
				self._reset()
				self.log.warning("Unexpected connection from {0!s}".format((event.peer.address)))
				raise network.CommandError("Unexpected connection from {0!s}".format((event.peer.address)))
	...
	def _receive(self, timeout=SERVER_TIMEOUT):
		...
		try:
			...
			try:
				...
				self.log.error("Unknown packet from {0!s}!".format((event.peer.address)))
			errstr = "Pickle/Security: {0!s}".format((e))
			print "[FATAL] {0!s}".format((errstr)) # print that even when no logger is enabled!
