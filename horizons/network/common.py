...
class Player(object):
	...
	def __str__(self):
		if self.name:
			return "Player(addr={0!s};proto={1:d};name={2!s})".format(self.address, self.protocol, self.name)
		else:
...
class Game(object):
	...
	def __str__(self):
		return "Game(uuid={0!s};maxpl={1:d};plcnt={2:d};pw={3:d};state={4!s})".format(self.uuid, self.maxplayers, self.playercnt, self.has_password(), Game.State(self.state))
