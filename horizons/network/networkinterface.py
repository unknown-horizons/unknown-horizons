...
class NetworkInterface(object):
	...
	def creategame(self, mapname, maxplayers, gamename, maphash="", password=""):
		...
		try:
			self._assert_connection()
			self.log.debug("[CREATE] mapname={0!s} maxplayers={1:d}".format(mapname, maxplayers))
	...
	def _on_change_color(self, game, plold, plnew, myself):
		self.log.debug("[ONCHANGECOLOR] {0!s}: {1!s} -> {2!s}".format(plnew.name, plold.color, plnew.color))
		if myself:
...
class MPGame(object):
	...
	def __str__(self):
		return "{0!s} ({1:d}/{2:d})".format(self.map_name, self.player_count, self.player_limit)
