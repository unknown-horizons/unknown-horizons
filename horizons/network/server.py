...
class Server(object):
    ...
    def run(self):
        logging.info("Starting up server on %s:%d" % (self.hostname,
                                                      self.port))
        try:
            ...
            raise network.NetworkException(
                "Unable to create network structure: %s" % (e))
    ...
    def onconnect(self, event):
        ...
        if player.protocol not in PROTOCOLS:
            logging.warning("[CONNECT] %s runs old or unsupported protocol" %
                            (player))
            self.fatalerror(
    ...
    def onreceive(self, event):
        ...
        if peer.data not in self.players:
            logging.warning("[RECEIVE] Packet from unknown player %s!" %
                            (peer.address))
        ...
        if len(event.packet.data) > self.capabilities['maxpacketsize']:
            logging.warning("[RECEIVE] Global packet size exceeded from "
                            "%s: size=%d" % (peer.address,
                                             len(event.packet.data)))
            self.fatalerror(
                player, __("You've exceeded the global packet size.") + " " +
        ...
        try:
            ...
            logging.warning("[RECEIVE] Per packet size exceeded from %s: %s" %
                            (player, e))
            self.fatalerror(
            ...
            logging.warning("[RECEIVE] Unknown or malformed packet "
                            "from %s: %s!" % (player, e))
            self.fatalerror(player, __("Unknown or malformed packet. "
                                       "Please check your game version"))
            return
        ...
        if packet.sid != player.sid:
            logging.warning(
                "[RECEIVE] Invalid session id for player %s (%s vs %s)!"
        ...
        if packet.__class__ not in self.callbacks:
            logging.warning("[RECEIVE] Unhandled network packet from %s - "
                            "Ignoring!" % (peer.address))
            return
    ...
    def oncreategame(self, player, packet):
        if packet.maxplayers < self.capabilities['minplayers']:
            raise network.SoftNetworkException(
                "You can't run a game with less than %d players"
                % (self.capabilities['minplayers']))
        if packet.maxplayers > self.capabilities['maxplayers']:
            raise network.SoftNetworkException(
                "You can't run a game with more than %d players"
                % (self.capabilities['maxplayers']))
        game = Game(packet, player)
        logging.debug("[CREATE] [%s] %s created %s" % (game.uuid,
                                                       player, game))
    ...
    def onlistgames(self, player, packet):
        ...
        for _game in self.games:
            ...
            if _game.is_full():
                continue
            if packet.clientversion != -1 and \
                    packet.clientversion != _game.creator.version:
                continue
    ...
    def onjoingame(self, player, packet):
        if player.game is not None:
            self.error(player,
                       __("You can't join a game while in another game"))
            return

        game = self.__find_game_from_uuid(packet)
        if game is None:
            self.error(player, __("Unknown game or game is running a "
                                  "different version"))
            return
        if not game.is_open():
        ...
        for _player in game.players:
            if _player.name == packet.playername:
                self.error(player, __("There's already a player with your "
                                      "name inside this game.") + " " +
                           __("Please change your name."))
                return
            if _player.color == packet.playercolor:
                self.error(player, __("There's already a player with your "
                                      "color inside this game.") + " " +
                           __("Please change your color."))
                return
            if _player.clientid == packet.clientid:
                self.error(player, __("There's already a player with your "
                                      "unique player ID inside this game. "
    ...
    def terminategame(self, game, player=None):
        logging.debug("[TERMINATE] [%s] (by %s)" % (
            game.uuid, player if player is not None else None))
        if game.creator.protocol >= 1 and game.is_open():
            # NOTE: works with protocol >= 1
            for _player in game.players:
                self.error(_player, __("The game has been terminated. "
                                       "The creator has left the game."),
            ...
            for _player in game.players:
                if _player.peer.state == enet.PEER_STATE_CONNECTED:
                    self.fatalerror(
                        _player,
                        __("One player has terminated their game. For "
                           "technical reasons, this currently means the game "
                           "cannot continue. We are very sorry about that."))
        self.call_callbacks('deletegame', game)

    def preparegame(self, game):
        logging.debug("[PREPARE] [%s] Players: %s" % (
            game.uuid, [unicode(i) for i in game.players]))
        game.state = Game.State.Prepare
        for _player in game.players:
            self.send(_player.peer, packets.server.cmd_preparegame())

    def startgame(self, game):
        logging.debug("[START] [%s] Players: %s" % (
            game.uuid, [unicode(i) for i in game.players]))
    ...
    def onchat(self, player, packet):
        ...
        if not game.is_open():
            return
        logging.debug("[CHAT] [%s] %s: %s" % (game.uuid, player,
                                              packet.chatmsg))
        for _player in game.players:
    ...
    def onchangename(self, player, packet):
        ...
        # ACK the change
        logging.debug("[CHANGENAME] [{0}] {1} -> {2}".format(
    ...
    def onchangecolor(self, player, packet):
        ...
        # ACK the change
        logging.debug("[CHANGECOLOR] [{0}] Player:{1} {2} -> {3}".format(
            game.uuid, player.name, player.color, packet.playercolor))
    ...
    def ontoggleready(self, player, packet):
        ...
        # ACK the change
        player.toggle_ready()
        logging.debug("[TOGGLEREADY] [{0}] Player:{1} {2} ready".
                      format(game.uuid, player.name,
                             "is not" if not player.ready else "is"))
        for _player in game.players:
            self.send(_player.peer, packets.server.data_gamestate(game))
    ...
    def onkick(self, player, packet):
        ...
        logging.debug("[KICK] [{0}] {1} got kicked".format(game.uuid,
                                                           kickplayer.name))
        for _player in game.players:
    ...
    def onsavegamedata(self, player, packet):
        ...
        for _player in game.players:
            if _player.sid == packet.psid:
                self.send(_player.peer, packets.server.savegame_data(
                    packet.data, player.sid, game.mapname))
