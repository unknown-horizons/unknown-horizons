...
class ProductionChain(object):
    """
    A production chain handles the building of buildings
     required to produce a resource.

    Production chains use the list of production lines and the available
    AbstractBuilding subclasses to figure out all ways of producing
    a certain resource and finding the right ratio of them to produce
    just enough of the resource. The result is a tree that can be used
    to produce the required resource.

    Each subtree reserves a portion of the total capacity of the buildings
    of the relevant type. This is a logical classification and doesn't affect
    the actual buildings in any way.
    Some subtrees can import the required resource from other islands using
    a mechanism similar to the previous one but in that case it acts as if
    the resource was produced without any subtrees. The imported amounts are
    added up over time and saved as an owed resource in the exporting
    settlement's resource manager (these restrictions are again just logical
    without affecting the way the settlements work in any way). That storage
    is realized by organizing DomesticTrade missions that transfer the
    resources to the right settlements.
    """
    ...
    def __init__(self, settlement_manager, resource_id, resource_producer):
        ...
        self.chain.assign_identifier('/{0:d},{1:d}'.format(
            self.settlement_manager.worldid, self.resource_id))

    def _get_chain(self, resource_id, resource_producer, production_ratio):
        """Return a ProductionChainSubtreeChoice if it is possible to produce
        the resource, None otherwise."""
        options = []
        if resource_id in resource_producer:
            for production_line, abstract_building in \
                    resource_producer[resource_id]:
                possible = True
                sources = []
                ...
                if possible:
                    options.append(ProductionChainSubtree(
                        self.settlement_manager, resource_id,
                        production_line, abstract_building, sources,
                        production_ratio))
        if not options:
            return None
        ...
        for abstract_building in AbstractBuilding.buildings.itervalues():
            for resource, production_line in \
                ...
                resource_producer[resource].append((production_line,
                                                    abstract_building))
        return ProductionChain(settlement_manager, resource_id,
                               resource_producer)

    def __str__(self):
        return 'ProductionChain({0:d}): {1:.5f}\n{2!s}'.format(
            self.resource_id,
    ...
    def reserve(self, amount, may_import):
        """Reserve currently available production capacity and import from
        other islands if allowed."""
...
class ProductionChainSubtreeChoice(object):
    ...
    log = logging.getLogger("ai.aiplayer.productionchain")
    coverage_resources = set([RES.COMMUNITY, RES.FAITH, RES.EDUCATION,
                              RES.GET_TOGETHER])
    ...
    def __init__(self, options):
        ...
        # given that 1 unit has to be produced at the root of the tree,
        # how much has to be produced here?
    ...
    def __str__(self, level=0):
        result = '{0!s}Choice between {1:d} options: {2:.5f}\n'.format(
    ...
    def get_root_import_level(self):
        ...
        return self.trade_manager.get_quota(
            self.identifier, self.resource_id) / self.production_ratio

    def get_final_production_level(self):
        """Return the total reserved production capacity of the resource per
        tick (includes import)."""
    ...
    def get_expected_cost(self, amount):
        """Return the expected utility cost of building enough buildings
        to produce a total
        of the given amount of the resource per tick."""
        return min(option.get_expected_cost(amount) for option in self.options)

    def _get_available_options(self):
        """Return a list of the currently available options to produce
        the resource."""
        available_options = []
        for option in self.options:
            if option.available:
    ...
    def build(self, amount):
        ...
        if amount < current_production + 1e-7 and self.resource_id not in \
        ...
        for i, option in enumerate(available_options):
            cost = option.get_expected_cost(amount - current_production +
                                            option.
        ...
        if not expected_costs:
            ...
            for option in zip(*sorted(expected_costs))[2]:
                ...
                # TODO: this amount should not include the part provided
    ...
    def reserve(self, amount, may_import):
        """Reserve currently available production capacity and import
        from other islands if allowed.
        Returns the total amount it can reserve or import."""
        total_reserved = 0.0
        for option in self._get_available_options():
            total_reserved += option.reserve(max(0.0, amount - total_reserved),
                                             may_import)

        # check how much we can import
        if may_import:
            required_amount = max(0.0, amount - total_reserved)
            self.trade_manager.request_quota_change(
                self.identifier, self.resource_id,
    ...
    def get_ratio(self, resource_id):
        """Return the ratio of the given resource needed given that 1 unit of
        the root resource is required."""
        return sum(option.get_ratio(resource_id) for option in self.options)


class ProductionChainSubtree(object):
    """An object of this type represents a subtree of buildings that need to
    be built in order to produce the given resource."""

    def __init__(self, settlement_manager, resource_id, production_line,
                 abstract_building, children, production_ratio):
        ...
        # AbstractBuilding instance
        self.children = children
        # [ProductionChainSubtreeChoice, ...]
    ...
    @property
    def available(self):
        """Return a boolean showing whether this subtree is currently
        available."""
        return self.settlement_manager.owner.settler_level >= \
    ...
    def get_expected_children_cost(self, amount):
        """Return the expected utility cost of building enough buildings in
        the subtrees to produce a total of the given amount of the resource
        per tick."""
        total = 0
        for child in self.children:
    ...
    def get_expected_cost(self, amount):
        ...
        production_needed = (amount - self.get_root_production_level()
                             ) * self.production_ratio
        root_cost = self.abstract_building.get_expected_cost(
            self.resource_id, production_needed, self.settlement_manager)
        if root_cost is None:
            return None
        return children_cost + root_cost

    def get_root_production_level(self):
        """Return the currently reserved production capacity of this subtree
        at the root."""
        return self.resource_manager.get_quota(
            self.identifier, self.resource_id,
    ...
    def get_final_production_level(self):
        ...
        for child in self.children:
            ...
            if min_child_production is None:
                ...
                min_child_production = min(min_child_production,
                                           production_level)
        if min_child_production is None:
    ...
    def need_more_buildings(self, amount):
        """Return a boolean showing whether more buildings of this specific
        type need to be built in order to produce at least the given amount
        of resource per tick."""
    ...
    def build(self, amount):
        """Build a building in order to get closer to the goal of producing
        at least the given amount of resource per tick at the bottleneck."""
        # try to build one of the lower level buildings
        #  (results in a depth first order)
        result = None
        for child in self.children:
            ...
            if result == BUILD_RESULT.ALL_BUILT:
                ...
                break
                # parent building has to be built before child
                #  (example: farm before field)
            else:
                return result  # an error or successful building

        if result == BUILD_RESULT.NEED_PARENT_FIRST or \
                self.need_more_buildings(amount):
            # build a building
            (result, building) = self.abstract_building.build(
                self.settlement_manager, self.resource_id)
            if result == BUILD_RESULT.OUT_OF_SETTLEMENT:
                return self.settlement_manager.production_builder.\
                    extend_settlement(building)
            return result
        return BUILD_RESULT.ALL_BUILT

    def reserve(self, amount, may_import):
        """Reserve currently available production capacity and
        import from other islands if allowed.
        Returns the total amount it can reserve or import."""
        total_reserved = amount
        for child in self.children:
            total_reserved = min(total_reserved, child.reserve(amount,
                                                               may_import))

        self.resource_manager.request_quota_change(
            self.identifier, True, self.resource_id,
            self.abstract_building.id, amount * self.production_ratio)
        total_reserved = min(total_reserved, self.resource_manager.get_quota(
            self.identifier, self.resource_id,
            self.abstract_building.id) / self.production_ratio)
        return total_reserved

    def need_to_build_more_buildings(self, amount):
        """Return a boolean showing whether more buildings in this subtree
        need to be built in order to produce at least the given amount
        of resource per tick."""
        for child in self.children:
        ...
        if not self.need_more_buildings(amount):
            return False
        return self.abstract_building.need_to_build_more_buildings(
            self.settlement_manager, self.resource_id)

    def get_ratio(self, resource_id):
        """Return the ratio of the given resource needed given that 1 unit of
        the root resource is required."""
        result = self.production_ratio if \
            self.resource_id == resource_id else 0
        return result + sum(child.get_ratio(resource_id) for
