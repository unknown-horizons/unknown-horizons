...
from horizons.world.buildability.simplecollectorareacache import \
    SimpleCollectorAreaCache
...
class ProductionBuilder(AreaBuilder):
    ...
    * plan: a dictionary of the form {(x, y): (purpose, extra data), ...}
     where purpose is one of the BUILDING_PURPOSE constants.
        Coordinates being in the plan means that the tile doesn't
        belong to another player.
    * collector_buildings: a list of every building in the settlement
      that provides general collectors (warehouse, storages)
    * production_buildings: a list of buildings in the settlement
      where productions should be paused and resumed at appropriate times
    * unused_fields: a dictionary where the key is a BUILDING_PURPOSE
      constant of a field and the value is a deque that holds the
        coordinates of unused field spots. {building purpose:
        deque([(x, y), ...]), ...}
    * last_collector_improvement_storage: the last tick when
      a storage was built to improve collector coverage
    * last_collector_improvement_road: the last tick when a new road
      connection was built to improve collector coverage
    """

    coastal_building_classes = [BUILDINGS.FISHER, BUILDINGS.BOAT_BUILDER,
    ...
    def __init(self, settlement_manager, last_collector_improvement_storage,
        ...
        self.personality = self.owner.personality_manager.get(
            'ProductionBuilder')
        self.last_collector_improvement_storage = \
            last_collector_improvement_storage
    ...
    def save(self, db):
        ...
        translated_last_collector_improvement_storage = \
            self.last_collector_improvement_storage - Scheduler().cur_tick
        # pre-translate for the loading process
        translated_last_collector_improvement_road = \
            self.last_collector_improvement_road - Scheduler().cur_tick
        # pre-translate for the loading process
        db("INSERT INTO ai_production_builder(rowid, settlement_manager,"
           " last_collector_improvement_storage, "
           "last_collector_improvement_road) VALUES(?, ?, ?, ?)",
           self.worldid, self.settlement_manager.worldid,
           translated_last_collector_improvement_storage,
           translated_last_collector_improvement_road)
        for (x, y), (purpose, _) in self.plan.iteritems():
            db("INSERT INTO ai_production_builder_plan("
               "production_builder, x, y, purpose)"
               " VALUES(?, ?, ?, ?)", self.worldid, x, y, purpose)

    def _load(self, db, settlement_manager):
        worldid, last_storage, last_road = \
            db("SELECT rowid, last_collector_improvement_storage, "
               "last_collector_improvement_road"
               " FROM ai_production_builder WHERE settlement_manager = ?",
        ...
        db_result = db("SELECT x, y, purpose FROM ai_production_builder_plan"
                       " WHERE production_builder = ?", worldid)
    ...
    def have_deposit(self, resource_id):
        """Returns True if there is a resource deposit of the
        relevant type inside the settlement."""
    ...
    def extend_settlement_with_storage(self, target_position):
        """Build a storage to extend the settlement towards
        the given position.
        ...
        for (x, y) in sorted(storage_spots):
            ...
            for (dx, dy) in storage_surrounding_offsets:
                coords = (x + dx, y + dy)
                if coords in coastline or coords not in self.plan \
    ...
    def get_collector_area(self):
        """Return the set of all coordinates that are reachable
        from at least one collector by road or open space."""
        if self.__collector_area_cache is not None \
                and self.last_change_id == self.__collector_area_cache[0]:
                ...
                for dx, dy in moves:
                    ...
                    if coords not in reachable and coords in coverage_area:
                        if coords in self.land_manager.roads or (
                                coords in self.plan and
                                coords not in self.land_manager.coastline and
                                self.plan[coords][0] == BUILDING_PURPOSE.NONE):
                            queue.append(coords)
                            reachable.add(coords)
                            if coords in self.plan and self.plan[
    ...
    def count_available_squares(self, size, max_num=None):
        """
        Count the number of available and usable
        (covered by collectors) size x size squares.

        @param size: the square side length
        @param max_num: if non-None then stop counting once
                        the number of total squares is max_num
        @return: (available squares, total squares)
        """

        key = (size, max_num)
        if key in self.__available_squares_cache and \
                self.last_change_id == self.__available_squares_cache[key][0]:
    ...
    def _refresh_unused_fields(self):
        """Refresh the unused_fields object to make sure no impossible
        fields spots are in the list."""
        self.unused_fields = {
        ...
        for coords, (purpose, _) in sorted(self.plan.iteritems()):
            usable = True
            # is every tile of the field spot still
            #  usable for new normal buildings
            for dx in xrange(3):
                for dy in xrange(3):
    ...
    def display(self):
        """Show the plan on the map unless it is disabled
        in the settings."""
    ...
    def _init_cache(self):
        """Initialize the cache that knows the last time
        the buildability of a rectangle
        may have changed in this area."""
    ...
    def register_change_list(self, coords_list, purpose, data):
        ...
        for coords in coords_list:
            if coords in self.land_manager.village or coords not in self.plan:
                continue
            if purpose == BUILDING_PURPOSE.NONE and self.plan[
                    coords][0] != BUILDING_PURPOSE.NONE:
                add_list.append(coords)
            elif purpose != BUILDING_PURPOSE.NONE and self.plan[
                    coords][0] == BUILDING_PURPOSE.NONE:
        ...
        super(ProductionBuilder, self).register_change_list(coords_list,
                                                            purpose, data)
        self.road_connectivity_cache.modify_area(coords_list)
        self.display()

    def handle_lost_area(self, coords_list):
        """Handle losing the potential land
        in the given coordinates list."""
        ...
        removed_list = []
        for coords, (purpose, _) in self.plan.iteritems():
            if purpose in [BUILDING_PURPOSE.POTATO_FIELD,
                           BUILDING_PURPOSE.PASTURE,
                           BUILDING_PURPOSE.SUGARCANE_FIELD,
                           BUILDING_PURPOSE.TOBACCO_FIELD,
                           BUILDING_PURPOSE.HERBARY]:
                rect = Rect.init_from_topleft_and_size_tuples(coords,
                                                              field_size)
    ...
    def handle_new_area(self):
        """Handle receiving more land to the production area
        (this can happen when the village area gives some up)."""
        new_coords_list = []
    ...
    collector_building_classes = [BUILDINGS.WAREHOUSE,
                                  BUILDINGS.STORAGE]
    field_building_classes = [BUILDINGS.POTATO_FIELD,
                              BUILDINGS.PASTURE,
                              BUILDINGS.SUGARCANE_FIELD,
                              BUILDINGS.TOBACCO_FIELD]
    production_building_classes = set([BUILDINGS.FISHER, BUILDINGS.LUMBERJACK,
                                       BUILDINGS.FARM, BUILDINGS.CLAY_PIT,
                                       BUILDINGS.BRICKYARD, BUILDINGS.WEAVER,
                                       BUILDINGS.DISTILLERY, BUILDINGS.MINE,
                                       BUILDINGS.SMELTERY, BUILDINGS.TOOLMAKER,
    ...
    def _handle_lumberjack_removal(self, building):
        """Release the unused trees around the lumberjack building
        being removed."""
        ...
        for coords in building.position.get_radius_coordinates(
                building.radius):
            if coords not in trees_used_by_others and coords in self.plan \
                    and self.plan[coords][0] == BUILDING_PURPOSE.TREE:
                coords_list.append(coords)
        self.register_change_list(coords_list, BUILDING_PURPOSE.NONE, None)

    def _handle_farm_removal(self, building):
        """Handle farm removal by removing planned fields and tearing existing
        ones that can't be serviced by another farm."""
        unused_fields = set()
        ...
        for coords in building.position.get_radius_coordinates(
            ...
            for farm in farms:
                if farm.worldid != building.worldid and \
                        object.position.distance(farm.position) <= farm.radius:
    ...
    def remove_building(self, building):
        ...
        if building.id in self.field_building_classes:
            ...
            Scheduler().add_new_object(Callback(self._refresh_unused_fields),
                                       self, run_in=0)
            Scheduler().add_new_object(Callback(partial(super(
                ProductionBuilder, self).remove_building,
                building)), self, run_in=0)
    ...
    def manage_production(self):
        """Pauses and resumes production buildings
        when they have full input and output inventories."""
        for building in self.production_buildings:
            ...
            for production in producer.get_productions():
                ...
                for resource_id, min_amount in\
                        production.get_produced_resources().iteritems():
                ...
                if all_full:
                    if not production.is_paused():
                        ...
                        self.log.info('%s paused a production at %s/%d',
                                      self, building.name, building.worldid)
                else:
                    if production.is_paused():
                        ToggleActive(producer, production).execute(
                            self.land_manager.session)
                        self.log.info('%s resumed a production at %s/%d',
                                      self, building.name, building.worldid)
    ...
    def __str__(self):
        return '{0!s}.PB({1!s}/{2!s})'.format(
            self.owner, self.settlement.get_component(
