...
class ResourceManager(WorldObject):
	...
	def get_required_upgrade_resources(self, resource_id, upgrade_limit):
		...
		needed = 0
		for residence in self.settlement_manager.settlement.buildings_by_id[BUILDINGS.RESIDENTIAL_CLASS]:
			if limit_left <= 0:
				break
...
class SingleResourceManager(WorldObject):
	...
	def _get_current_production(self):
		...
		buildings = self.settlement_manager.settlement.buildings_by_id[self.building_id]
