...
class PersonalityManager(object):
    """This class handles the loading of personality data
    for the AI players."""
    ...
    def __init__(self, player):
        self.player = player
        self._personality = player.session.random.choice(
            self.available_personalities)
        self.log.info('%s assigned personality %s', player,
                      self._personality.__name__)

    def save(self, db):
        db("INSERT INTO ai_personality_manager(rowid, personality) "
           "VALUES(?, ?)", self.player.worldid,
    ...
    def _load(self, db, player):
        ...
        if self._personality is None:
            self.log.debug('%s had invalid personality %s', self.player,
                           personality)
            self._personality = self.player.session.random.choice(
                self.available_personalities)
        self.log.info('%s loaded with personality %s', self.player,
                      self._personality.__name__)
