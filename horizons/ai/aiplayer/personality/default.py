...
class DefaultPersonality:
	class SettlementFounder:
		...
		# minimum amount of usable free land on an island to consider turning it into a feeder island

		# found a settlement on a random island that is at least as large as the first element;
		...
		# extra distance to add to the usual warehouse to island distance when choosing an island
		nearby_enemy_penalty = 100  # importance of keeping our islands away from other players' islands
		extra_enemy_island_distance = 1
		# extra distance to add to the usual island to other player's island distance
	...
	class InternationalTradeManager:
		little_money = 3000
		# cutoff to decide that we really need to get more money by selling resources on other
		# players' islands
		buy_coefficient_rich = 30
		# value coefficient of buying resources when we have more than little_money gold
		buy_coefficient_poor = 10
		# value coefficient of buying resources when we have less than or equal to little_money gold
	...
	class VillageBuilder(AreaBuilder):
		...
		# the penalty for building two roads right next to each other (per complete road, not segment)
		...
		# the initial plan calls for this number of residences per coverage building
		# (may or may not be optimized away)
	...
	class LandManager:
		max_section_side = 22
		# minimize the number of village sections by considering this to be its maximum side length
	...
	class ResourceManager:
		...
		# try to always have this much food in settlement inventory in settlements with a village
		default_feeder_island_brick_requirement = 20
		# try to always have this much bricks in feeder island inventory (active on level > 0)
		...
		# maximum number of houses whose upgrades should be accounted for in the reserve_time
		buy_threshold = 0.66666
		# when more than buy_threshold * needed_amount of resource exists then stop buying
		sell_threshold = 1.33333
		# when less than sell_threshold * needed_amount of resource exists then stop selling
		low_requirement_threshold = 5
		# when we need less than or equal to this amount of resource then disregard buy_threshold
	...
	class SettlementManager:
		...
		# more than this proportion of residences must have settler status to be able
		...
		# tax rates and upgrade rights in settlements where the first sailors have
		...
		# tax rates and upgrade rights in settlements where bricks production exists
	...
	class FoundSettlement:
		...
		# add a penalty of this constant * distance to a warehouse to the warehouse penalty
		max_options = 10  # maximum number of options to consider

	class FeederChainGoal:
		extra_priority = 1
		# extra priority given to goals that are supposed to produce resources for
	...
	class ClayDepositCoverageGoal:
		...
		# the importance of alignment when choosing a location for a storage to get closer to a deposit

	class DoNothingGoal:
		enabled = True
		default_priority = 1500
		# mean priority; changing this will influence which goals are more important than doing nothing
	...
	class EnlargeCollectorAreaGoal:
		...
		# the importance of alignment when choosing a location for a storage to enlarge collector coverage
		max_interesting_collector_area = 100
		# maximum collector area (of 3x3 squares) we are interested in when considering whether
		# to enlarge the area
		max_collector_area_unreachable = 10
		# maximum collector area (of 3x3 squares) that doesn't have to be reachable
		# when considering whether to enlarge the area
		overlap_precision = 0.2
		# probability that a tile in the radius will be checkedfor match for useful coordinates
	...
	class FoundFeederIslandGoal:
		...
		# if there are less than this many free 3x3 squares in a settlement then a feeder
		# island is needed
		usable_feeder_island_cutoff = 30
		# if there are less than this many free 3x3 on a feeder island then another feeder
		# island may be needed
	...
	class ImproveCollectorCoverageGoal:
		...
		min_free_space = 20
		# if there is less than this much free space for a resource then it doesn't matter that
		# the building in badly covered
		max_good_collector_utilization = 0.7
		# if the collector building is used more than this then don't attempt to improve coverage
		# by connecting more production buildings

		max_reasonably_served_buildings = 3
		# maximum number of buildings a storage can reasonably serve (not a hard limit)
		collector_extra_distance = 6.0
		# constant distance on top of the actual distance a collector has to move (accounts for breaks)
		alignment_coefficient = 0.001
		# the importance of alignment when choosing a location for a storage to improve collector coverage

		collector_improvement_road_expires = 1500
		# minimum number of ticks between collector improvement road connections
	...
	class StorageSpaceGoal(ImproveCollectorCoverageGoal):
		...
		# maximum storage capacity to go for when the inventory starts to get full
		full_storage_threshold = 5
		# when there is less than this amount of free space for a resource then we might need more space
	...
	class AbstractVillageBuilding:
		...
		# build a coverage building if at least this amount of the assigned residences have been built
	...
	class BoatBuilderEvaluator:
		...
		# the larger this value, the larger the effect of alignment on the placement

	class BrickyardEvaluator:
		alignment_importance = 0.02
		# the larger this value, the larger the effect of alignment on the placement
		collector_distance_importance = 0.1
		# importance of the distance to the nearest collector in the range [0, 1]
		distance_penalty = 2
		# when no clay pit is in reach then apply a penalty of this times the radius

	class CharcoalBurnerEvaluator:
		alignment_importance = 0.02
		# the larger this value, the larger the effect of alignment on the placement
		lumberjack_distance_importance = 0.05
		# importance of the distance to the nearest lumberjack in the range [0, 1]
		iron_mine_distance_importance = 0.1
		# importance of the distance to the nearest iron mine in the range [0, 1]
		distance_penalty = 2
		# when no lumberjack or iron mine is in reach then apply a penalty of this times the radius

	class DistilleryEvaluator:
		alignment_importance = 0.02
		# the larger this value, the larger the effect of alignment on the placement
		farm_distance_importance = 0.3
		# importance of the distance to the nearest relevant farm in the range [0, 1]
		distance_penalty = 2
		# when no relevant farm is in reach then apply a penalty of this times the radius
	...
	class LumberjackEvaluator:
		...
		shared_tree = 1
		# number of points for a shared tree in range (at least one lumberjack already using it)
	...
	class SmelteryEvaluator:
		...
		# importance of the distance to the nearest charcoal burner in the range [0, 1]
		distance_penalty = 2
		# when no collector or charcoal burner is in reach then apply a penalty of this times the radius

	class ToolmakerEvaluator:
		alignment_importance = 0.02
		# the larger this value, the larger the effect of alignment on the placement
		smeltery_distance_importance = 0.4
		# importance of the distance to the nearest smeltery in the range [0, 1]
		lumberjack_distance_importance = 0.1
		# importance of the distance to the nearest lumberjack in the range [0, 1]
		charcoal_burner_distance_importance = 0.4
		# importance of the distance to the nearest charcoal burner in the range [0, 1]
		distance_penalty = 2
		# when no smeltery, lumberjack, or charcoal burner is in reach then apply a penalty
	...
	class AbstractFireStation:
		...
		# build a fire station if at least this amount of the assigned residences have been built

	class AbstractDoctor:
		fraction_of_assigned_residences_built = 0.4
		# build a doctor if at least this amount of       the assigned residences have been built
