...
class SettlementManager(WorldObject):
	...
	* trade_manager: TradeManager instance
	...
	def _set_taxes_and_permissions_prefix(self, prefix):
		...
		sailor_upgrades = getattr(self.personality, '{0!s}_sailor_upgrades'.format(prefix))
		pioneer_upgrades = getattr(self.personality, '{0!s}_pioneer_upgrades'.format(prefix))
		settler_upgrades = getattr(self.personality, '{0!s}_settler_upgrades'.format(prefix))
	...
	def __str__(self):
		return '{0!s}.SM({1!s}/{2!s})'.format(self.owner, self.settlement.get_component(NamedComponent).name if hasattr(self, 'settlement') else 'unknown', self.worldid if hasattr(self, 'worldid') else 'none')
