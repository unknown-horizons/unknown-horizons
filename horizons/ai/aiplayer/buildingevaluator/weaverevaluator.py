...
class WeaverEvaluator(BuildingEvaluator):
	def __init__(self, production_builder, builder, distance_to_farm, distance_to_collector, alignment):
		super(WeaverEvaluator, self).__init__(production_builder, builder)
	...
	def create(cls, production_builder, x, y, orientation):
		...
		return WeaverEvaluator(production_builder, builder, distance_to_farm, distance_to_collector, alignment)
	...
	def execute(self):
		...
		self.production_builder.plan[sorted(self.builder.position.tuple_iter())[0]] = (PRODUCTION_PURPOSE.WEAVER, self.builder)
