...
class Mission(WorldObject):
	...
	def __str__(self):
		return '{0!s} {1!s}({2:d})'.format(self.owner if hasattr(self, 'owner') else 'unknown player',
...
class ShipMission(Mission):
	...
	def __str__(self):
		return super(ShipMission, self).__str__() + (' using {0!s}'.format((self.ship if hasattr(self, 'ship') else 'unknown ship')))
