...
class ShipMission(Mission):
    def __init__(self, success_callback, failure_callback, ship):
        super(ShipMission, self).__init__(success_callback,
                                          failure_callback, ship.owner)
        self.__init(ship)
    ...
    def load(self, db, worldid, success_callback, failure_callback, ship):
        super(ShipMission, self).load(db, worldid, success_callback,
                                      failure_callback, ship.owner)
        self.__init(ship)
    ...
    def move_resource(cls, ship, settlement, resource_id, amount):
        """Move up to amount tons of resource_id from the ship
        to the settlement."""
        if amount > 0:
            missing = ship.get_component(StorageComponent).inventory.alter(
                resource_id, -amount)
            overflow = settlement.get_component(
                StorageComponent).inventory.alter(
                    resource_id, amount - missing)
            ship.get_component(StorageComponent).inventory.alter(
                resource_id, overflow)
        elif amount < 0:
            missing = settlement.get_component(
                StorageComponent).inventory.alter(resource_id, amount)
            overflow = ship.get_component(StorageComponent).inventory.alter(
                resource_id, missing - amount)
            settlement.get_component(StorageComponent).inventory.alter(
                resource_id, overflow)

    def _unload_all_resources(self, settlement):
        """copy the inventory because otherwise we would be modifying
        it while iterating"""
    ...
    def _move_to_warehouse_area(self, position, success_callback,
        ...
        try:
            self.ship.move(area, success_callback,
