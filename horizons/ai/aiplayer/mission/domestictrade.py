...
class DomesticTrade(ShipMission):
    """
    Given a ship and two settlement managers the ship is taken to
    the first one, loaded with resources, and then moved to the other
    one to unload the resources.
    """
    ...
    def __init__(self, source_settlement_manager,
                 destination_settlement_manager, ship,
                 success_callback, failure_callback):
        super(DomesticTrade, self).__init__(success_callback,
    ...
    def save(self, db):
        ...
        db("INSERT INTO ai_mission_domestic_trade(rowid,"
           " source_settlement_manager, destination_settlement_manager, "
           "ship, state) VALUES(?, ?, ?, ?, ?)",
    ...
    def _load(self, db, worldid, success_callback, failure_callback):
        db_result = db("SELECT source_settlement_manager, "
                       ...
                       " ship, state FROM ai_mission_domestic_trade WHERE "
                       "rowid = ?", worldid)[0]
        ...
        if self.state == self.missionStates.moving_to_source_warehouse:
            self.ship.add_move_callback(Callback(
                self._reached_source_warehouse_area))
            ...
            self.ship.add_move_callback(Callback(
                self._reached_destination_warehouse_area))
            self.ship.add_blocked_callback(Callback(
