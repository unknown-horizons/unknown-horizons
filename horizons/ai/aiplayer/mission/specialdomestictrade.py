...
class SpecialDomesticTrade(ShipMission):
    ...
    def __init__(self, source_settlement_manager, destination_settlement_manager,
    ...
    def save(self, db):
        ...
        db("INSERT INTO ai_mission_special_domestic_trade(rowid, source_settlement_manager,"
    ...
    def _load(self, db, worldid, success_callback, failure_callback):
        db_result = db("SELECT source_settlement_manager, destination_settlement_manager, ship, state"
            " FROM ai_mission_special_domestic_trade WHERE rowid = ?", worldid)[0]
        ...
        super(SpecialDomesticTrade, self).load(db, worldid, success_callback, failure_callback,
    ...
    def _move_to_source_settlement(self):
        self._move_to_warehouse_area(self.source_settlement_manager.settlement.warehouse.position,
            ...
            Callback(self._move_to_source_settlement), 'Unable to move to the source settlement ({})'
            .format(self.source_settlement_manager.settlement.get_component(NamedComponent).name))
    ...
    def _load_resources(self):
        ...
        for resource_id, limit in destination_resource_manager.resource_requirements.iteritems():
            ...
            tradable_amount = min(self.ship.get_component(StorageComponent).inventory.get_limit(resource_id),
