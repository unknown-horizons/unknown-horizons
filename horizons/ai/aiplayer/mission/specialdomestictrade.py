...
class SpecialDomesticTrade(ShipMission):
    """
    Given a ship and two settlement managers the ship will go to
    the source destination, load the most useful resources for
    the destination settlement, and then unload at the
    ...
    def __init__(self, source_settlement_manager,
                 ...
                 ship, success_callback, failure_callback):
    ...
    def save(self, db):
        ...
        db("INSERT INTO ai_mission_special_domestic_trade(rowid, "
           "source_settlement_manager, destination_settlement_manager, ship, "
           "state) VALUES(?, ?, ?, ?, ?)", self.worldid,
           self.source_settlement_manager.worldid,
           self.destination_settlement_manager.worldid,
           self.ship.worldid, self.state.index)
    ...
    def _load(self, db, worldid, success_callback, failure_callback):
        db_result = db("SELECT source_settlement_manager, "
                       "destination_settlement_manager, ship, state"
                       " FROM ai_mission_special_domestic_trade WHERE "
                       "rowid = ?", worldid)[0]
        ...
        super(SpecialDomesticTrade, self).load(
            db, worldid, success_callback, failure_callback,
        ...
        if self.state is self.missionStates.moving_to_source_settlement:
            self.ship.add_move_callback(Callback(
                self._reached_source_settlement))
            ...
            self.ship.add_move_callback(Callback(
                self._reached_destination_settlement))
            self.ship.add_blocked_callback(Callback(
    ...
    def start(self):
        ...
        self._move_to_source_settlement()
        self.log.info(
            '%s started a special domestic trade mission from %s to %s '
            'using %s', self, self.source_settlement_manager.settlement.
    ...
    def _load_resources(self):
        source_resource_manager = \
            self.source_settlement_manager.resource_manager
        source_inventory = self.source_settlement_manager.settlement.\
            get_component(StorageComponent).inventory
        destination_resource_manager = \
            self.destination_settlement_manager.resource_manager
        destination_inventory = self.destination_settlement_manager.\
        ...
        for resource_id, limit in destination_resource_manager.\
            ...
            if source_inventory[resource_id] <= \
                ...
                continue
                # the source settlement doesn't have a surplus of the resource

            price = self.owner.session.db.get_res_value(resource_id)
            tradable_amount = min(
                self.ship.get_component(StorageComponent).inventory.
                get_limit(resource_id),
                limit - destination_inventory[resource_id],
                source_inventory[resource_id] - source_resource_manager.
                resource_requirements[resource_id])
            options.append((tradable_amount * price, tradable_amount,
                            resource_id))
    ...
    def _reached_source_settlement(self):
        self.log.info('%s reached the first warehouse area (%s)', self,
    ...
    def _move_to_destination_settlement(self):
        self._move_to_warehouse_area(
            ...
            'Unable to move to the destination settlement (%s)'
            % self.destination_settlement_manager.settlement.get_component(
                NamedComponent).name)
    ...
    def _reached_destination_settlement(self):
        ...
        self.log.info('%s reached the destination warehouse area (%s)', self,
                      self.destination_settlement_manager.settlement.
                      get_component(NamedComponent).name)
