...
class PrepareFoundationShip(ShipMission):
    """Given a ship and a settlement manager it moves the ship to
    the warehouse and loads
    it with the resources required to start another settlement.
    """

    missionStates = Enum('created', 'moving')

    def __init__(self, settlement_manager, ship, feeder_island,
                 success_callback, failure_callback):
        super(PrepareFoundationShip, self).__init__(success_callback,
    ...
    def save(self, db):
        ...
        db("INSERT INTO ai_mission_prepare_foundation_ship(rowid, "
           "settlement_manager, ship, feeder_island, state) "
           "VALUES(?, ?, ?, ?, ?)", self.worldid,
           self.settlement_manager.worldid, self.ship.worldid,
    ...
    def _load(self, db, worldid, success_callback, failure_callback):
        db_result = db("SELECT settlement_manager, ship, feeder_island, "
                       "state FROM ai_mission_prepare_foundation_ship WHERE "
                       "rowid = ?", worldid)[0]
        ...
        super(PrepareFoundationShip, self).load(db, worldid, success_callback,
                                                failure_callback,
    ...
    def _move_to_destination_area(self):
        self._move_to_warehouse_area(self.warehouse.position,
    ...
    def _load_foundation_resources(self):
        ...
        if self.feeder_island:
            max_amounts = {RES.BOARDS: personality.
                           max_new_feeder_island_boards,
        ...
        for resource_id, max_amount in max_amounts.iteritems():
            self.move_resource(self.ship, self.settlement_manager.settlement,
                               resource_id, self.ship.get_component
