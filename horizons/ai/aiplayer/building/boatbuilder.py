...
class AbstractBoatBuilder(AbstractBuilding):
	...
	def register_buildings(cls):
		cls._available_buildings[BUILDINGS.BOATBUILDER] = cls

class BoatBuilderEvaluator(BuildingEvaluator):
	@classmethod
	def create(cls, area_builder, x, y, orientation):
		builder = area_builder.make_builder(BUILDINGS.BOATBUILDER, x, y, True, orientation)
		if not builder:
			return None
		...
		value = float(Entities.buildings[BUILDINGS.BOATBUILDER].radius) / distance_to_collector + alignment * personality.alignment_importance
