...
class DistilleryEvaluator(BuildingEvaluator):
    ...
    def create(cls, area_builder, x, y, orientation):
        builder = BasicBuilder.create(BUILDINGS.DISTILLERY, (x, y),
                                      orientation)
        ...
        if distance_to_collector is None:
            return None
            # require distilleries to have a collector building in range

        personality = area_builder.owner.personality_manager.get(
            'DistilleryEvaluator')
        distance_penalty = Entities.buildings[BUILDINGS.DISTILLERY].radius * \
        ...
        distance = cls._weighted_distance(distance_to_collector, [(
            personality.farm_distance_importance,
            distance_to_farm)], distance_penalty)
        value = float(Entities.buildings[BUILDINGS.DISTILLERY].radius) / \
            distance + alignment \
