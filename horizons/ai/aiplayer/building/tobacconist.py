...
class TobacconistEvaluator(BuildingEvaluator):
    ...
    def create(cls, area_builder, x, y, orientation):
        builder = BasicBuilder.create(BUILDINGS.TOBACCONIST, (x, y),
                                      orientation)
        ...
        for building in area_builder.settlement.buildings_by_id.get(
            ...
            if distance <= Entities.buildings[BUILDINGS.TOBACCONIST].radius:
                ...
                for provider in building.get_providers():
                    if isinstance(provider, Entities.buildings[
                             BUILDINGS.TOBACCO_FIELD]):
                        tobacco_producer = True
                        break
                if tobacco_producer:
                    distance_to_farm = distance if distance_to_farm is \
                        None or distance < distance_to_farm else \
                        distance_to_farm
        ...
        if distance_to_collector is None:
            return None
            # require tobacconists to have a collector building in range

        personality = area_builder.owner.personality_manager.get(
            'TobacconistEvaluator')
        distance_penalty = Entities.buildings[BUILDINGS.TOBACCONIST].radius * \
        ...
        distance = cls._weighted_distance(
            ...
            distance_penalty)
        value = (float(Entities.buildings[BUILDINGS.TOBACCONIST].radius) /
