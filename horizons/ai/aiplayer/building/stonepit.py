...
class AbstractStonePit(AbstractBuilding):
	def iter_potential_locations(self, settlement_manager):
		building_class = Entities.buildings[BUILDINGS.STONE_PIT]
		for building in settlement_manager.settlement.buildings_by_id.get(BUILDINGS.STONE_DEPOSIT, []):
			if building.get_component(StorageComponent).inventory[RES.STONE_DEPOSIT]:
