...
class WeaponsmithEvaluator(BuildingEvaluator):
	...
	def create(cls, area_builder, x, y, orientation):
		builder = BasicBuilder.create(BUILDINGS.WEAPONSMITH, (x, y), orientation)
		...
		personality = area_builder.owner.personality_manager.get('WeaponsmithEvaluator')
		distance_penalty = Entities.buildings[BUILDINGS.WEAPONSMITH].radius * personality.distance_penalty
		...
		distance = cls._weighted_distance(distance_to_collector, [(personality.smeltery_distance_importance, distance_to_smeltery),
			...
			distance_penalty)
		value = float(Entities.buildings[BUILDINGS.WEAPONSMITH].radius) / distance + alignment * personality.alignment_importance
		return WeaponsmithEvaluator(area_builder, builder, value)
...
decorators.bind_all(AbstractWeaponsmith)
decorators.bind_all(WeaponsmithEvaluator)
