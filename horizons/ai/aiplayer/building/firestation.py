...
class AbstractFireStation(AbstractBuilding):
    def iter_potential_locations(self, settlement_manager):
        ...
        for coords in village_builder.special_building_assignments[
                BUILDING_PURPOSE.FIRE_STATION].iterkeys():
            if (coords not in spots_in_settlement or
                    village_builder.plan[coords][1][0] >
...
class FireStationEvaluator(BuildingEvaluator):
    ...
    def create(cls, production_builder, x, y, orientation):
        ...
        village_builder = settlement_manager.village_builder
        builder = BasicBuilder.create(BUILDINGS.FIRE_STATION, (x, y),
                                      orientation)
        ...
        for residence_coords in assigned_residences:
            if village_builder.plan[residence_coords][0] == \
                    BUILDING_PURPOSE.RESIDENCE:
                not_serviced += 1

        if (not_serviced <= 0 or not_serviced < total *
                settlement_manager.owner.personality_manager
                .get('AbstractFireStation')
