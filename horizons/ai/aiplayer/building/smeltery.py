...
class SmelteryEvaluator(BuildingEvaluator):
	...
	def create(cls, area_builder, x, y, orientation):
		...
		distance_to_charcoal_burner = cls._distance_to_nearest_building(area_builder, builder,
			BUILDINGS.CHARCOAL_BURNER)
		if distance_to_collector is None and (distance_to_charcoal_burner is None
				or distance_to_iron_mine is None):
			return None
		...
		distance = cls._weighted_distance(distance_to_iron_mine,
			...
			distance_penalty)
		value = (float(Entities.buildings[BUILDINGS.SMELTERY].radius) / distance + alignment
