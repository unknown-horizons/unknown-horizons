...
class ImproveCollectorCoverageGoal(SettlementGoal):
    ...
    @property
    def active(self):
        return super(ImproveCollectorCoverageGoal,
                     self).active and self._is_active
    ...
    def _get_problematic_collector_coverage_buildings(self):
        ...
        for building in self.production_builder.production_buildings:
            for production in building.get_component(
                ...
                # take paused time into account because the AI pauses
                # the production when the output storage is full
                amount_paused = history[
                    PRODUCTION.STATES.inventory_full.index] + history[
                        PRODUCTION.STATES.paused.index]
                if amount_paused < self.personality.min_bad_collector_coverage:
                    continue
                for resource_id in production.get_produced_resources():
                    if self.settlement.get_component(
    ...
    def update(self):
        if (self.production_builder.last_collector_improvement_road +
                self.personality.collector_improvement_road_expires >
                Scheduler().cur_tick):
    ...
    def _build_extra_road_connection(self, building, collector_building):
        collector_coords = set(
            coords for coords in self.production_builder
            .iter_possible_road_coords(
        ...
        for resource_id, amount in cost.iteritems():
            if resource_id == RES.GOLD:
                if self.owner.get_component(StorageComponent).inventory[
                        resource_id] < amount:
                    return BUILD_RESULT.NEED_RESOURCES
            elif self.settlement.get_component(StorageComponent).inventory[
                    resource_id] < amount:
                return BUILD_RESULT.NEED_RESOURCES
    ...
    def _build_extra_road(self):
        """Build an extra road between a storage building
        and a producer building."""
        ...
        for building in self.production_builder.collector_buildings:
            if (building.get_utilization_history_length() < 1000 or
                    building.get_collector_utilization() <
                    self.personality.max_good_collector_utilization):
        ...
        for building in self._problematic_buildings:
            for collector_building in usable_collectors:
                distance = building.loading_area.distance(
                    collector_building.position)
                if distance > collector_building.radius:
                    continue  # out of range anyway
                # TODO: check whether the link already exists
                potential_road_connections.append((
                    distance * collector_building.get_collector_utilization(),
                    building, collector_building))

        # try the best link from the above list
        for _, building, collector_building in sorted(
                potential_road_connections):
            result = self._build_extra_road_connection(
                building, collector_building)
            if result == BUILD_RESULT.OK:
                self.production_builder.last_collector_improvement_road = \
                    current_tick
                self.log.info(
                    '%s connected %s at %d, %d with %s at %d, %d',
                    self, building.name, building.position.origin.x,
                    building.position.origin.y, collector_building.name,
                    collector_building.position.origin.x,
                    collector_building.position.origin.y)
            return result
        self.log.info('%s found no good way to connect buildings that'
    ...
    def _build_extra_storage(self):
        ...
        reachable = dict.fromkeys(self.land_manager.roads)
        ...
        for building in self._problematic_buildings:
            ...
            for coords, dist in distance.iteritems():
                if dist is not None:
                    if building.loading_area.distance(coords) <= \
                            storage_radius:
                        reachable[coords].append((building.worldid, dist))

        options = []
        storage_class = Entities.buildings[BUILDINGS.STORAGE]
        storage_spots = self.island.terrain_cache.\
            get_buildability_intersection(
        ...
        for coords, building_distances in reachable.iteritems():
            ...
            for coords in builder.position.tuple_iter():
                for building_worldid, distance in reachable[coords]:
                    if building_worldid not in actual_distance or \
    ...
    def execute(self):
        ...
        if result == BUILD_RESULT.IMPOSSIBLE:
            if (self.production_builder.last_collector_improvement_storage +
                    self.personality.collector_improvement_storage_expires <=
                    Scheduler().cur_tick):
