...
class Goal(object):
	...
	def _translate_build_result(cls, result):
		...
		assert False, 'Unable to translate BUILD_RESULT {0:d} to a GOAL_RESULT'. \
			format(result)
	...
	def __str__(self):
		return 'Goal({0:d}): {1!s}({2:d})'. \
			format(self.priority, self.__class__.__name__, self.sequence_number)
