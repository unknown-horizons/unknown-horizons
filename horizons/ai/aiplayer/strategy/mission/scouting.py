...
class ScoutingMission(FleetMission):
    ...
    missionStates = Enum.get_extended(FleetMission.missionStates,
                                      'sailing_to_target', 'going_back')

    def __init__(self, success_callback, failure_callback, ships,
                 target_point):
        super(ScoutingMission, self).__init__(success_callback,
    ...
    def __init(self, target_point, starting_point):
        ...
        self._state_fleet_callbacks = {
            ...
            self.missionStates.going_back: Callback(
                self.report_success, "Ships arrived at the target"),
    ...
    def save(self, db):
        ...
        db("INSERT INTO ai_scouting_mission (rowid, starting_point_x,"
           " starting_point_y, target_point_x, target_point_y) "
           "VALUES(?, ?, ?, ?, ?)", self.worldid, self.starting_point.x,
           self.starting_point.y, self.target_point.x, self.target_point.y)

    def _load(self, worldid, owner, db, success_callback, failure_callback):
        super(ScoutingMission, self)._load(db, worldid, success_callback,
    ...
    def sail_to_target(self):
        if not self.target_point:
            self.target_point = self.owner.session.world. \
                get_random_possible_ship_position()
        try:
    ...
    def create(cls, success_callback, failure_callback, ships,
               target_point=None):
        return ScoutingMission(success_callback, failure_callback, ships,
                               target_point)
