...
class ChaseShipsAndAttack(FleetMission):
    ...
    def __init__(self, success_callback, failure_callback, ships, target_ship):
        super(ChaseShipsAndAttack, self).__init__(success_callback,
    ...
    def __init(self, target_ship):
        ...
        self.combatIntermissions = {
            self.missionStates.sailing_to_target: (self.sail_to_target,
                                                   self.flee_home),
            self.missionStates.in_combat: (self.check_ship_alive,
                                           self.flee_home),
    ...
    def save(self, db):
        ...
        db("INSERT INTO ai_mission_chase_ships_and_attack "
           "(rowid, target_ship_id) VALUES(?, ?)",
    ...
    def _load(self, worldid, owner, db, success_callback, failure_callback):
        ...
        (target_ship_id,) = db("SELECT target_ship_id FROM "
    ...
    def sail_to_target(self):
        self.log.debug("Player %s, Mission %s, 1/2 set off to ship %s at %s",
                       ...
                       self.target_ship.get_component(NamedComponent).name,
                       self.target_ship.position)
        try:
            self.fleet.move(Circle(self.target_ship.position,
                                   self.target_range),
                            self._state_fleet_callbacks[
    ...
    def was_reached(self):
        if self.target_ship.in_ship_map:
            if any((ship.position.distance(self.target_ship.position) <=
                    self.target_range + 1 for ship in self.fleet.get_ships())):
    ...
    def in_combat(self):
        if not self.session.world.diplomacy.are_enemies(
                self.owner, self.target_ship.owner):
            self.report_failure("Target ship was not hostile. "
                                "Aborting mission.")
            return
        self.combat_phase = True
        self.log.debug("Player %s, Mission %s, 2/2 in combat",
                       self.owner.name, self.__class__.__name__)
        self.state = self.missionStates.in_combat
        ...
        if self.fleet.size() > 0:
            try:
                ...
                return_point = self.unit_manager.get_warehouse_area(
                    home_settlement, 10)
                self.fleet.move(return_point, self._state_fleet_callbacks[
    ...
    def create(cls, success_callback, failure_callback, fleet, target_ship):
        return ChaseShipsAndAttack(success_callback, failure_callback, fleet,
                                   target_ship)
