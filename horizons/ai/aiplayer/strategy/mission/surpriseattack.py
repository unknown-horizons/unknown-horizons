...
class SurpriseAttack(FleetMission):
    ...
    def __init(self, target_point, return_point, enemy_player):
        ...
        self.combatIntermissions = {
            ...
            self.missionStates.breaking_diplomacy: (self.break_diplomacy,
                                                    self.flee_home),
        ...
        self._state_fleet_callbacks = {
            self.missionStates.sailing_to_target: Callback(
                self.break_diplomacy),
            self.missionStates.going_back: Callback(
                self.report_success,
    ...
    def save(self, db):
        ...
        db("INSERT INTO ai_mission_surprise_attack "
           "(rowid, enemy_player_id, target_point_x,"
           ...
           self.target_point.center.y, self.target_point.radius,
           self.return_point.x, self.return_point.y)
    ...
    def _load(self, worldid, owner, db, success_callback, failure_callback):
        ...
        db_result = db(
            "SELECT enemy_player_id, target_point_x, target_point_y,"
    ...
    def in_combat(self):
        ...
        self.log.debug("Player %s, Mission %s, 3/4 in combat",
                       self.owner.name, self.__class__.__name__)
        self.state = self.missionStates.in_combat

    def go_back(self):
        self.log.debug("Player %s, Mission %s, 4/4 going back after "
                       "combat to point %s", self.owner.name,
                       self.__class__.__name__, self.return_point)
        try:
            self.fleet.move(self.return_point,
                            self._state_fleet_callbacks
    ...
    def flee_home(self):
        if self.fleet.size() > 0:
            try:
                ...
                self.report_failure("Combat was lost, ships couldn't "
                                    "flee home")
        else:
