...
class StrategyManager(object):
    """
    StrategyManager object is responsible for handling major decisions
    in game such as sending fleets to battle, keeping track of diplomacy
    between players, declare wars.
    """
    ...
    @property
    def conditions(self):
        """conditions are held in behavior manager since they are a part
        of behavior profile (just like actions and strategies)"""
    ...
    def calculate_player_wealth_balance(self, other_player):
        ...
        Wealth balance of 1.2 means that self.owner is 1.2 times wealthier
         than other_player.
        ...
        for player in [self.owner, other_player]:
            ...
            for settlement in player.settlements:
                resources_value += sum(
                    (self.session.db.get_res_value(resource) * amount for
                     resource, amount in settlement.get_component(
                        StorageComponent).inventory.itercontents() if
                     self.session.db.get_res_value(resource)))
            resource_values.append(resources_value)
        ai_resources, enemy_resources = resource_values

        ai_gold = self.owner.get_component(StorageComponent).inventory[
            RES.GOLD]
        enemy_gold = other_player.get_component(StorageComponent).inventory[
            RES.GOLD]
    ...
    def calculate_player_power_balance(self, other_player):
        ...
        Power balance of 1.2 means that self.owner is 1.2 times stronger
         than other_player
    ...
    def calculate_player_terrain_balance(self, other_player):
        ...
        Terrain balance of 1.2 means that self.owner has 1.2 times larger
         terrain than other_player
        """
        ...
        # it favors having 3 islands of total size X, than 2 of
        #  total size X (or bigger)
        return (float(
            ai_terrain) / float(enemy_terrain)) * island_count_balance

    def calculate_player_balance(self, player, trimming_factor=10.0,
                                 linear_boundary=10.0):
        """
        Calculate power balance between self.owner and other player.

        trimming_factor: Since any balance returns values of (0, inf)
        we agree to assume
        ...
        @type player: Player
        @param trimming_factor: trim actual balance values to range
                                [1./trimming_factor, trimming_factor]
                                e.g. [0.1, 10.0]
        @type trimming_factor: float
        @param linear_boundary: boundaries of new balance scale
                                [-linear_boundary, linear_boundary],
                                e.g. [-10.0, 10.0]
        @type linear_boundary: float
        @return: unified balance for various variables
        @rtype: collections.namedtuple
        """
        wealth_balance = self.owner.strategy_manager.\
            calculate_player_wealth_balance(player)
        power_balance = self.owner.strategy_manager.\
            calculate_player_power_balance(player)
        terrain_balance = self.owner.strategy_manager.\
            calculate_player_terrain_balance(player)
        balance = {
        ...
        balance = dict(((key, trim_value(value, 1. / trimming_factor,
                                         trimming_factor)) for
                        key, value in balance.iteritems()))
        balance = dict(((key, map_balance(value, trimming_factor,
                                          linear_boundary)) for
                        key, value in balance.iteritems()))

        return collections.namedtuple(
            'Balance', 'wealth, power, terrain')(**balance)
    ...
    def save(self, db):
        ...
        for condition, mission in self.conditions_being_resolved.iteritems():
            db("INSERT INTO ai_condition_lock (owner_id, condition,"
               " mission_id) VALUES(?, ?, ?)",
    ...
    def _load(self, db):
        for class_name, db_table in self.missions_to_load.iteritems():
            db_result = db("SELECT m.rowid FROM %s m, ai_fleet_mission f"
                           " WHERE f.owner_id = ? and m.rowid = f.rowid" %
                           db_table, self.owner.worldid)
        ...
        db_result = db("SELECT condition, mission_id FROM ai_condition_lock "
                       "WHERE owner_id = ?", self.owner.worldid)
        for (condition, mission_id) in db_result:
            self.conditions_being_resolved[condition] = \
                WorldObject.get_object_by_id(mission_id)

    def report_success(self, mission, msg):
        self.log.info(
            "Player: %s|StrategyManager|Mission %s was a success: %s",
            self.owner.worldid, mission, msg)
        self.end_mission(mission)

    def report_failure(self, mission, msg):
        self.log.info(
            "Player: %s|StrategyManager|Mission %s was a failure: %s",
            self.owner.worldid, mission, msg)
        self.end_mission(mission)

    def end_mission(self, mission):
        self.log.info("Player: %s|StrategyManager|Mission %s ended",
                      self.owner.worldid, mission)
    ...
    def start_mission(self, mission):
        self.log.info("Player: %s|StrategyManager|Mission %s started",
                      self.owner.worldid, mission)
    ...
    def unlock_condition(self, mission):
        """values (FleetMission) are unique so it's possible
        to remove them this way:"""
    ...
    def get_missions(self, condition=None):
        """
        Get missions filtered by certain condition
         (by default return all missions)
        """
        if condition:
    ...
    def request_to_pause_mission(self, mission, **environment):
        ...
        @rtype: bool
        """
        # TODO: make that decision based on environment
    ...
    def get_ships_for_mission(self):
        ...
        rules = (filters.ship_state(self.owner.ships,
                                    (self.owner.shipStates.idle,)),
    ...
    def handle_strategy(self):
        ...
        other_players = [player for player in
                         self.session.world.players if player != self.owner]
        ...
        for player in other_players:
            # Prepare environment
            self.log.debug("Conditions occurring against player %s",
                           player.name)
            ...
            for condition in self.conditions.keys():
                ...
                if condition.get_identifier(**environment) in \
                        self.conditions_being_resolved:
                    self.log.debug("  %s: Locked",
                                   condition.__class__.__name__)
                    continue

                condition_outcome = condition.check(**environment)
                self.log.debug("  %s: %s", condition.__class__.__name__,
                               ("Yes" if condition_outcome else "No"))
        ...
        if occuring_conditions:
            ...
            selected_condition, selected_outcome = max(
                occuring_conditions,
                key=lambda (condition, outcome):
                self.conditions[condition] * outcome['certainty'])

            self.log.debug("Selected condition: %s",
                           selected_condition.__class__.__name__)
            ...
            mission = self.owner.behavior_manager.request_strategy(
                **environment)
            if mission:
                self.start_mission(mission)
                if selected_condition.lockable:
                    self.lock_condition(selected_condition.get_identifier(
                        **environment), mission)
