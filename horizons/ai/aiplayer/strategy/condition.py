...
class Condition(object):
    """
    Condition's goal is to aid StrategyManager in figuring out what kind
    of strategy/mission is applicable to given state of world, e.g. instead
    of having a really long method that decides what kind of reasoning should
    be done at given point, we have a collection of Conditions
    (with priorities) that are easier to handle.
    """
    ...
    def check(self, **environment):
        """
        Based on information contained in **environment, determine wheter
    ...
    def get_identifier(self, **environment):
        """
        Based on information contained in **environment return an Unique
        identifier for given condition.
        User for checking whether given condition is already being resolved
        by a mission in progress.
    ...
    def check(self, **environment):
        ...
        if my_islands & enemy_islands:
            return {'player': other_player,
                    'certainty': self.default_certainty,
                    'strategy_name': 'player_shares_island',
                    'type': BehaviorManager.strategy_types.offensive}
        else:
            return None
...
class ConditionNeutral(Condition):
    ...
    What it aims to do is not only find if given player is neutral,
    but also sort them, i.e. penalize if given neutral is ally with
    our enemies etc. This way in case of any diplomatic actions
    it's possible to have a "safe" ally
    """
...
class ConditionPirateRoutinePossible(Condition):
    """
    Currently always occurs, when pirate has more conditions/strategies
     to work on, this may change.
    """
    def __init__(self, owner):
    ...
    def check(self, **environment):
        return {'certainty': self.default_certainty,
                'strategy_name': 'pirate_routine',
                'type': BehaviorManager.strategy_types.idle}

    def get_identifier(self, **environment):
        return super(ConditionPirateRoutinePossible, self).get_identifier(
            **environment)
