...
from horizons.ai.aiplayer.strategy.mission.chaseshipsandattack import \
    ChaseShipsAndAttack
...
class BehaviorComponent(object):
    """This is an abstract BehaviorComponent -
    a building block for AI Behavior.
    """
    ...
    def __init__(self, owner):
        ...
        # Witchery below is a way to have certainty() always return
        # the same certainty if it's not defined per behavior.
        self._certainty = defaultdict(lambda: (
...
# (division below is not related to the way dictionaries
# in BehaviorManager are named (offensive, idle, defensive))


class BehaviorDoNothing(BehaviorComponent):
    """Behavior that does nothing. Used mainly for idle actions
    (we don't want to scout too often).
    """
...
class BehaviorPirateRoutine(BehaviorComponent):
    ...
    Responsible for pirate ships routine when no one is around.
    States change in a loop:
    idle -> moving_random -> going_home -> idle
    """

    sail_home_chance = 0.3
    # sail_home_chance to sail home, 1-sail_home_chance to sail randomly
    ...
    def trading_ships_in_sight(self, **environment):
        ...
        for ship in ship_group:
            ...
            self.log.debug('BehaviorPirateRoutine: Ship:%s '
                           'trading_ships_in_sight' %
                           ship.get_component(NamedComponent).name)

    def _arrived(self, ship):
        """
        Callback function executed once ship arrives at the
        destination after certain action.
        Practically only changes ship state to idle.
        """
        owner = ship.owner
        self.log.debug('Player %s: Ship %s: arrived at destination '
                       'after "%s"' % (
                           owner.name,
                           ship.get_component(NamedComponent).name,
                           owner.ships[ship]))
    ...
    def _sail_home(self, pirate_ship):
        ...
        try:
            ...
            owner.ships[pirate_ship] = owner.shipStates.idle
            self.log.debug('Pirate %s: Ship %s: unable to move home at %s' % (
                owner.worldid,
                pirate_ship.get_component(NamedComponent).name,
                owner.home_point))
    ...
    def _sail_random(self, pirate_ship):
        ...
        try:
            ...
            self.log.debug(
                'Pirate %s: Ship %s(%s): moving random at %s'
                % (owner.worldid, pirate_ship.get_component(
                    NamedComponent).name, owner.ships[pirate_ship], point))
        except MoveNotPossible:
            owner.ships[pirate_ship] = owner.shipStates.idle
            self.log.debug('Pirate %s: Ship %s: unable to move random at '
                           '%s' % (owner.worldid, pirate_ship.get_component(
                               NamedComponent).name, point))


# Common certainty functions for offensive actions
def certainty_power_balance_exp(**environment):
    """Return power_balance^2, altering the exponent will impact
    the weight certainty has.
    """
    return BehaviorComponent.default_certainty * (
        environment['power_balance'] ** 2)
...
class BehaviorRegular(BehaviorComponent):
    ...
    def __init__(self, owner):
        ...
        self._certainty['fighting_ships_in_sight'] = \
            certainty_power_balance_exp
        self._certainty['player_shares_island'] = \
            self._certainty_player_shares_island
    ...
    def pirate_ships_in_sight(self, **environment):
        """Attacks pirates only if they are enemies already and
        the power balance is advantageous.
        """
        pirates = environment['enemies']
        ship_group = environment['ship_group']
        power_balance = environment['power_balance']

        # It's enough to check if first pirate is hostile,
        #  since there is only one pirate player.
        if self.session.world.diplomacy.are_enemies(self.owner,
                                                    pirates[0].owner):
            # Let each ship attack it's closest enemy
            #  to maximize dps (in a way)
            ship_pairs = UnitManager.get_closest_ships_for_each(ship_group,
                                                                pirates)
            for ship, pirate in ship_pairs:
                ship.attack(pirates[0])
            BehaviorComponent.log.info('%s: Attacked pirate ship',
                                       self.__class__.__name__)
        else:
            BehaviorComponent.log.info('%s: Pirate ship was not hostile',
                                       self.__class__.__name__)

    def fighting_ships_in_sight(self, **environment):
        """Attacks frigates only if they are enemies already and
        the power balance is advantageous.
        """
    ...
    def working_ships_in_sight(self, **environment):
        ...
        if self.session.world.diplomacy.are_enemies(self.owner,
            ...
            BehaviorComponent.log.info('%s: Enemy worker was not hostile',
                                       self.__class__.__name__)
    ...
    def hostile_player(self, **environment):
        ...
        mission = ChaseShipsAndAttack.create(
            self.owner.strategy_manager.report_success,
            self.owner.strategy_manager.report_failure,
            idle_ships, target_ship)
...
class BehaviorAggressive(BehaviorComponent):
    ...
    def neutral_player(self, **environment):
        ...
        # Send a surprise attack if there are ships available,
        #  otherwise simply declare war
        if idle_ships:
            target_point = self.unit_manager.get_warehouse_area(
...
class BehaviorSmart(BehaviorComponent):
    ...
    def fighting_ships_in_sight(self, **environment):
        ...
        ship_pairs = UnitManager.get_closest_ships_for_each(ship_group,
                                                            enemies)
        for ship, enemy_ship in ship_pairs:
            if self.session.world.diplomacy.are_enemies(ship.owner,
                                                        enemy_ship.owner):
                ...
                BehaviorComponent.log.info('%s: Enemy ship %s was not hostile',
                                           self.__class__.__name__,
                                           ship.get_component(
                                               NamedComponent).name)
    ...
    def working_ships_in_sight(self, **environment):
        ...
        if self.session.world.diplomacy.are_enemies(self.owner,
                                                    enemies[0].owner):
            # working ships won't respond with fire, each ship should attack
            # the closest one, and chase them if necessary.
            ship_pairs = UnitManager.get_closest_ships_for_each(ship_group,
                                                                enemies)
            for ship, enemy_ship in ship_pairs:
                range_function = CombatManager.close_range
...
# (you can think of it as of respect,
# or "relationship_score" values towards other player)
# Each AI values different traits in others. Based on that value AI
#  can break diplomacy
...
class BehaviorDiplomatic(BehaviorComponent):
    ...
    def calculate_relationship_score(self, balance, weights):
        """Calculate total relationship_score based on balances
        and their weights.
        Count only balances that have weight defined
        (this way "default" weight is 0)
        """
        return sum((getattr(balance, key) * value for key, value in
                    weights.iteritems()))
    ...
    def handle_diplomacy(self, parameters, **environment):
        ...
        relationship_score = self.calculate_relationship_score(balance,
                                                               self.weights)
        action = self._get_action(relationship_score, **parameters)
        self.log.debug("%s vs %s | Dipomacy: balance:%s, "
                       "relationship_score:%s, action:%s",
                       self.owner.name, player.name, balance,
                       relationship_score, action)
    ...
    def _perform_action(self, action, **environment):
        ...
        # ideally this shouldn't automatically change diplomacy
        # for both players (i.e. add_pair) but do it for one side only.
    ...
    def _get_quadratic_function(cls, mid, root, peek=1.0):
        """
        Functions for border distributions such as enemy or ally
         (left or right parabola).
        @param mid: value on axis X that is to be center of the parabola
        @type mid: float
        @param root: value on axis X which is a crossing point of axis OX
        ...
        # (e.g. if mid is 1.0 and root is 1.5 ->
        #  make original x^2 function 2 times narrower
    ...
    def _get_action(self, relationship_score, **parameters):
        ...
        if 'neutral' in parameters:
            ...
            possible_actions.append((
                ...
                self.get_neutral_function(**neutral_params)(
                    relationship_score), ))
    ...
    def hostile_player(self, **environment):
        """Calculate balance, and change diplomacy towards a player
        to neutral or ally.
        This has a very small chance though, since BehaviorEvil
        enjoys to be in a war.
        """
...
class BehaviorRegularPirate(BehaviorComponent):
    ...
    def __init__(self, owner):
        ...
        self._certainty[
            'fighting_ships_in_sight'] = certainty_power_balance_exp
        self._certainty['pirate_routine'] = self._certainty_pirate_routine

    def fighting_ships_in_sight(self, **environment):
        """Attacks frigates only if they are enemies already
        and the power balance is advantageous.
        """
        enemies = environment['enemies']
        ship_group = environment['ship_group']
        power_balance = environment['power_balance']
        ...
        if power_balance < self.power_balance_threshold:
            BehaviorComponent.log.info(
                '%s: Enemy ship was too strong, did not attack',
                self.__class__.__name__)
            return
    ...
    def pirate_routine(self, **environment):
        ...
        mission = PirateRoutine.create(
            self.owner.strategy_manager.report_success,
            self.owner.strategy_manager.report_failure, idle_ships)
        BehaviorComponent.log.info(
            'BehaviorRegularPirate: pirate_routine request')
        return mission
...
class BehaviorAggressivePirate(BehaviorComponent):
    ...
    def fighting_ships_in_sight(self, **environment):
        """Attacks frigates only if they are enemies.
        Does not care about power balance.
        """
...
class BehaviorBreakDiplomacy(BehaviorComponent):
    ...
    def fighting_ships_in_sight(self, **environment):
        ...
        BehaviorComponent.log.info('Player:%s broke diplomacy with %s' % (
...
class BehaviorCoward(BehaviorComponent):
    ...
    def __init__(self, owner):
        ...
        self._certainty[
            'pirate_ships_in_sight'] = certainty_power_balance_inverse
...
def certainty_are_enemies(**environment):
    """returns 0.0 if two players are enemies already,
    default certainty otherwise.
    """
    enemies = environment['enemies']
    ship_group = environment['ship_group']
    ...
    return 0.0 if player.session.world.diplomacy.are_enemies(
        player, enemy_player) else BehaviorComponent.default_certainty
...
class BehaviorPirateHater(BehaviorComponent):
    ...
    def pirate_ships_in_sight(self, **environment):
        ...
        if not self.session.world.diplomacy.are_enemies(self.owner,
                                                        enemies[0].owner):
                AddEnemyPair(self.owner, enemies[0].owner).execute(
                    self.session)
        BehaviorComponent.log.info('I feel urgent need to wipe out them '
                                   'pirates.')
