...
class PathNodes(object):
    """
    Abstract class; used to derive list of path nodes from,
    which is used for pathfinding.
    We encapsulate this code here to keep it centralized.
    """
    log = logging.getLogger("world.pathfinding")

    # TODO: currently all paths have speed 1,
    # since we don't have a real velocity-system yet.
    NODE_DEFAULT_SPEED = 1.0

    def __init__(self):
        # store a collection of path_nodes here. the type of this var has to
        # be supported by the pathfinding algo
        pass
...
class ConsumerBuildingPathNodes(PathNodes):
    ...
    Interface:
    self.nodes: {(x, y): speed, ...} of the home_building,
                where the collectors can walk
    """
...
class IslandPathNodes(PathNodes):
    ...
    Interface:
    self.nodes: List of nodes on island, where the terrain allows
    to be walked on
    self.road_nodes: dictionary of nodes, where a road is built on

    (un)register_road has to be called for each coord, where a road
    is built on (destroyed)
    reset_tile_walkablity has to be called when the terrain changes
    the walkability
    ...
    def __init__(self, island):
        ...
        # we keep this up to date, so that path finding can use it
        # and we don't have
    ...
    def is_walkable(self, coord):
        ...
        NOTE: nature tiles (trees..) are considered to be walkable
              (or else they could be used as
    ...
    def reset_tile_walkability(self, coord):
        """Reset the status of the walkability of a coordinate
        in the list of walkable tiles
        of the island. Does not change the tile itself.
        You need to call this when a tile changes,
        e.g. when a building is built on it. this
        is currently done in add/remove_building
        @param coord: tuple: (x, y)"""
        actually_walkable = self.is_walkable(coord)
        # TODO: this lookup on a list is O(n),
        # use different data structure here
