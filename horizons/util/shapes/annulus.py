...
class Annulus(Shape):
    ...
    def contains(self, point):
        ...
        return self.min_radius <= point.distance(
            self.center) <= self.max_radius

    def __str__(self):
        return "Annulus(center={0},min_radius={1},max_radius={2})".format(
    ...
    def tuple_iter(self):
        for x in xrange(self.center.x - self.max_radius,
                        self.center.x + self.max_radius + 1):
            for y in xrange(self.center.y - self.max_radius,
                            self.center.y + self.max_radius + 1):
                if self.min_radius <= self.center.distance((
                        x, y)) <= self.max_radius:
                    yield (x, y)
