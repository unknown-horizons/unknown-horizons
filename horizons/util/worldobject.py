...
class WorldObject(ChangeListener):
    ...
    def __init__(self, worldid=None, **kwargs):
        """
        @param worldid: worldid to assign. Use None to get an autogenerated
                        one.
        """
    ...
    def __init(self, worldid=None):
        self.worldid = worldid if worldid is not None else \
            WorldObject.__next_id
        assert self.worldid not in WorldObject.__objects,\
            "WorldObject ID(" + str(worldid) + ") already in use by " + \
            str(self.get_object_by_id(worldid))
        WorldObject.__objects[self.worldid] = self
        # Make sure that new WorldIDs are always higher than every
        # other WorldObject
