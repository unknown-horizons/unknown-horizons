...
class LastActivePlayerSettlementManager(object):
    ...
    Provides it as global reference, but stores as weak reference as
    ...
    def _on_scroll(self):
        """Called when view changes. Scrolling and zooming
        can change cursor position."""
    ...
    def _on_new_settlement_created(self, msg):
        """if the player has created a new settlement,
        it is the current one, even

        if the mouse hasn't hovered over it. Required when immediately
        entering build menu."""
