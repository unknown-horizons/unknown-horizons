...
class SavegameUpgrader(object):
	...
	def _upgrade_to_rev65(self, db):
		...
		for island_x, island_y, island_string in db('SELECT x, y, file FROM island ORDER BY rowid'):
			island_strings.append(island_string + ':{0:d}:{1:d}'.format(island_x, island_y))
	...
	def _upgrade(self):
		...
		if rev < VERSION.SAVEGAMEREVISION :
			...
			self.log.warning('Discovered old savegame file, auto-upgrading: {0!s} -> {1!s}'.format(rev, VERSION.SAVEGAMEREVISION))
