...
from horizons.messaging import UpgradePermissionsChanged, \
    SettlementInventoryUpdated
from horizons.util.changelistener import ChangeListener
from horizons.util.inventorychecker import InventoryChecker
from horizons.component.componentholder import ComponentHolder
from horizons.component.tradepostcomponent import TradePostComponent
from horizons.component.storagecomponent import StorageComponent
from horizons.world.buildability.settlementcache import \
    SettlementBuildabilityCache
from horizons.world.production.producer import Producer, GroundUnitProducer,\
    ShipProducer
...
class Settlement(ComponentHolder, WorldObject, ChangeListener,
                 ResourceHandler):
    """The Settlement class describes a settlement and stores all the
    necessary information like name, current inhabitants,
    lists of tiles and houses, etc belonging to the village."""
    ...
    def __init(self, session, owner, upgrade_permissions, tax_settings):
        ...
        self.buildings = []
        self.ground_map = {}
        # this is the same as in island.py. it uses hard references
        #  to the tiles too
        self.produced_res = defaultdict(int)
        # dictionary of all resources, produced at this settlement
        self.buildings_by_id = defaultdict(list)
        self.warehouse = None
        # this is set later in the same tick by the warehouse itself
        #  or load() here
    ...
    def init_buildability_cache(self, terrain_cache):
        self.buildability_cache = SettlementBuildabilityCache(terrain_cache,
                                                              self.ground_map)
    ...
    def inhabitants(self):
        ...
        (sum of inhabitants of its buildings)"""
    ...
    def get_residentials_of_lvl_for_happiness(self, level, min_happiness=0,
                                              max_happiness=101):
        is_valid_residential = lambda building: (
            hasattr(building, 'happiness') and
    ...
    def balance(self):
        ...
        return self.cumulative_taxes + self.get_component(
            TradePostComponent).sell_income - self.cumulative_running_costs - \
    ...
    def save(self, db, islandid):
        ...
        for res, amount in self.produced_res.iteritems():
            db("INSERT INTO settlement_produced_res "
               "(settlement, res, amount) VALUES(?, ?, ?)",
               self.worldid, res, amount)
        for level in xrange(TIER.CURRENT_MAX + 1):
            db("INSERT INTO settlement_level_properties (settlement, level,"
               " upgrading_allowed, tax_setting) VALUES(?, ?, ?, ?)",
               self.worldid, level, self.upgrade_permissions[level],
               self.tax_settings[level])

        # dump ground data via json, it's orders of magnitude faster
        #  than sqlite
    ...
    def load(cls, db, worldid, session, island):
        ...
        owner = db("SELECT owner FROM settlement WHERE rowid = ?",
                   worldid)[0][0]
        upgrade_permissions = {}
        tax_settings = {}
        for level, allowed, tax in db(
                "SELECT level, upgrading_allowed, tax_setting"
                " FROM settlement_level_properties WHERE settlement = ?",
                worldid):
            upgrade_permissions[level] = allowed
            tax_settings[level] = tax
        self.__init(session, WorldObject.get_object_by_id(owner),
                    upgrade_permissions, tax_settings)

        # load the settlement tile map
        tile_data = db("SELECT data FROM settlement_tiles WHERE rowid = ?",
                       worldid)[0][0]
        coords_list = [tuple(raw_coords) for raw_coords in
                       json.loads(tile_data)]
        ...
        for building_id, building_type in db(
        ...
        for res, amount in db("SELECT res, amount FROM "
                              "settlement_produced_res WHERE settlement = ?",
                              worldid):
    ...
    def add_building(self, building, load=False):
        ...
        if building.has_component(Producer) and not \
                building.has_component(ShipProducer) and not \
                building.has_component(GroundUnitProducer):
            finished = self.settlement_building_production_finished
            building.get_component(
                Producer).add_production_finished_listener(finished)
        if not load and not building.buildable_upon and \
                self.buildability_cache:
            self.buildability_cache.modify_area(
                [coords for coords in building.position.tuple_iter()])
    ...
    def remove_building(self, building):
        ...
        if building not in self.buildings:
            self.log.debug("Building %s can not be removed from settlement",
                           building.id)
            return
        ...
        if building.has_component(Producer) and not \
            ...
            finished = self.settlement_building_production_finished
            building.get_component(
                Producer).remove_production_finished_listener(finished)
        if not building.buildable_upon and self.buildability_cache:
            self.buildability_cache.add_area(
                [coords for coords in building.position.tuple_iter()])
    ...
    def count_buildings(self, id):
        """Returns the number of buildings in the settlement
    ...
    def __init_inventory_checker(self):
        ...
        self.__inventory_checker = InventoryChecker(SettlementInventoryUpdated,
                                                    storage, 4)
