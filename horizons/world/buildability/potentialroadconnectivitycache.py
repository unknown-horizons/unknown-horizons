...
class PotentialRoadConnectivityCache(object):
    """
    Query whether a toad connection between two sets of coordinates
    is possible.

    This class is used by the AI to figure out whether it might
    be possible to build a road between two sets of coordinates.
    Potentially because the area contains some part of the AI's plan
    and the land it has a plan for may be either owned by the AI or not
    yet owned by anyone.
    """
    ...
    def modify_area(self, coords_list):
        ...
        the possibility of building a road may have changed.
        It figures out whether it is possible to build a road on (x, y)
        and updates the underlying ConnectedAreaCache accordingly.
        """
        ...
        for coords in coords_list:
            if coords not in self._settlement_ground_map:
                ...
                if self._area_builder.plan[coords][0] == \
                        BUILDING_PURPOSE.NONE:
                    if coords not in self.area_numbers:
                        add_list.append(coords)
                else:
                    assert self._area_builder.plan[coords][0] != \
                        BUILDING_PURPOSE.ROAD
                    if coords in self.area_numbers:
                        remove_list.append(coords)
            else:
                if coords in self.area_numbers:
    ...
    def is_connection_possible(self, coords_set1, coords_set2):
        """Return True if and only if it is possible to connect
        the two coordinate sets.

        More specifically, it returns True if and only if it
        is possible to build a toad from some (x1, y1) in coords_set1
        to some (x2, y2) in coords_set2 entirely within the area.
        This is done cheaply using the underlying ConnectedAreaCache.
        """
