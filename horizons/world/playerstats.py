...
class PlayerStats(WorldObject):
    ...
    def _collect_info(self):
            ...
            for building in settlement.buildings:
                ...
                if building.id == BUILDINGS.RESIDENTIAL:
                    ...
                    for production in building.get_component(
                        ...
                        if production.get_state() is \
                                PRODUCTION.STATES.producing:
                            produced_resources = \
                            ...
                            if RES.HAPPINESS in produced_resources:
                                ...
                                for resource_id in \
                                    ...
                                    settler_resources_provided[resource_id] +=\
                                        happiness / production.\
                                        get_production_time()

                # resources held in buildings
                if building.has_component(StorageComponent) and \
                        building.id not in [BUILDINGS.WAREHOUSE,
                                            BUILDINGS.STORAGE,
                                            BUILDINGS.MAIN_SQUARE]:
                    for resource_id, amount in building.get_component(
                ...
                if building.has_component(CollectingComponent):
                    for collector in building.get_component(
                            CollectingComponent).get_local_collectors():
                        for resource_id, amount in collector.get_component(
                                StorageComponent).inventory.itercontents():
                            total_resources[resource_id] += amount

            # resources in settlement inventories
            for resource_id, amount in settlement.get_component(
                    StorageComponent).inventory.itercontents():
                available_resources[resource_id] += amount

            # land that could be built on (the building on it may need
        ...
        for ship in self.player.session.world.ships:
            if ship.owner is self.player:
                ...
                if ship.has_component(SelectableComponent):
                    for resource_id, amount in ship.get_component(
                            StorageComponent).inventory.itercontents():
                        available_resources[resource_id] += amount
        ...
        self._calculate_settler_score(settlers, settler_buildings,
                                      settler_resources_provided)
    ...
    def _calculate_settler_score(self, settlers, settler_buildings,
                                 settler_resources_provided):
        total = 0
    ...
    def _calculate_building_score(self, buildings):
        ...
        for building_id, amount in buildings.iteritems():
            for resource_id, res_amount in Entities.buildings[
                    building_id].costs.iteritems():
                resources[resource_id] += amount * res_amount
        for resource_id, amount in resources.iteritems():
            if resource_id == RES.GOLD:
                total += amount
                # for some reason the value of gold is 0 by default
            else:
    ...
    # the resource exists but isn't usable so it is worth less
    ...
    def _calculate_resource_score(self, available_resources, total_resources):
        total = 0
        for resource_id, amount in available_resources.iteritems():
            if resource_id in self.overridden_resource_values:
                # natural resources have 0 value by default
                total += amount * self.overridden_resource_values[resource_id]
            else:
                value = self.db.get_res_value(resource_id)
                if value is not None:
                    # happiness and some coverage resources have no value
                    total += amount * value
        for resource_id, amount in total_resources.iteritems():
            ...
            if resource_id in self.overridden_resource_values:
                ...
                if value is not None:
                    # happiness and some coverage resources have no value
                    total += extra_amount * value * \
                             self.unavailable_resource_coefficient
        self.resource_score = int(total * self.resource_score_coefficient)

    unit_value = {UNITS.FRIGATE: 1.5, UNITS.PLAYER_SHIP: 1,
                  UNITS.USABLE_FISHER_BOAT: 1, UNITS.FISHER_BOAT: 0.05}
    ...
    def _calculate_total_score(self):
        self.total_score = (self.settler_score + self.building_score +
                            self.resource_score + self.unit_score +
                            self.land_score + self.money_score)
