...
class StorageBuilding(StorageResourceHandler,
    ...
    These objects don't have a storage themselves,
    but use the settlement storage.
    """
    def __init__(self, x, y, owner, instance=None, **kwargs):
        super(StorageBuilding, self).__init__(x=x, y=y, owner=owner,
    ...
    def initialize(self):
        ...
        inv.adjust_limit(self.session.db.get_storage_building_capacity(
            self.id))
    ...
    def get_utilization_history_length(self):
        ...
        if collecting_comp.get_local_collectors():
            return collecting_comp.get_local_collectors()[0]\
                .get_utilization_history_length()

    def get_collector_utilization(self):
        collectors = self.get_component(
            CollectingComponent).get_local_collectors()
        if not collectors:
            return None
        return sum(collector.get_utilization() for collector in
                   collectors) / float(len(collectors))


class StorageTent(StorageBuilding, BuildableSingle):
    """Can't inherit from Buildable* in StorageBuilding because
    of mro issues."""
    pass
