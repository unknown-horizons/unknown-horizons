...
class BasicBuilding(ComponentHolder, ConcreteObject):
    ...
    walkable = False
    # whether we can walk on this building (True for e.g. streets, trees..)
    buildable_upon = False
    # whether we can build upon this building
    is_building = True
    tearable = True
    ...
    @param action_set_id: use this action set id.
                          None means choose one at random
    """

    def __init__(self, x, y, rotation, owner, island, level=None, **kwargs):
        self.__pre_init(owner, rotation, Point(x, y), level=level)
        super(BasicBuilding, self).__init__(x=x, y=y, rotation=rotation,
                                            owner=owner,
        ...
        if settlements:
            ...
            self.settlement = self.island.add_settlement(self.position,
                                                         self.radius, owner)
    ...
    def __pre_init(self, owner, rotation, origin, level=None):
        """Here we face the awkward situation of requiring a fourth
        init function. It is called like __init, but before other parts
        are inited via super(). This is necessary since some attributes
        are used by these other parts."""
        self.owner = owner
        if level is None:
    ...
    def __init(self, remaining_ticks_of_month=None):
        ...
        # shape where collector get resources

        origin = self.position.origin
        self._instance, action_name = self.getInstance(
            self.session, origin.x, origin.y,
            rotation=self.rotation, action_set_id=self._action_set_id,
            world_id=str(self.worldid))

        if self.has_running_costs:  # Get payout every 30 seconds
            interval = self.session.timer.get_ticks(
                GAME.INGAME_TICK_INTERVAL)
            run_in = remaining_ticks_of_month if \
                remaining_ticks_of_month is not None else interval
            Scheduler().add_new_object(self.get_payout, self,
                                       run_in=run_in, loops=-1,
    ...
    def running_costs_active(self):
        """Returns whether the building currently pays the running
        costs for status 'active'"""
        return (self.running_costs > self.running_costs_inactive)

    def get_payout(self):
        """Gets the payout from the settlement in form of its running costs"""
        self.owner.get_component(StorageComponent).inventory.alter(
            RES.GOLD, -self.running_costs)
    ...
    def remove(self):
        ...
        # NOTE: removing layers from the renderer here will affect
        # others players too!

    def save(self, db):
        super(BasicBuilding, self).save(db)
        db("INSERT INTO building (rowid, type, x, y, rotation, location, "
           "level)VALUES (?, ?, ?, ?, ?, ?, ?)", self.worldid,
           self.__class__.id, self.position.origin.x,
        ...
        if self.has_running_costs:
            ...
            db("INSERT INTO remaining_ticks_of_month(rowid, ticks) "
               "VALUES(?, ?)", self.worldid, remaining_ticks)
    ...
    def load(self, db, worldid):
        ...
        owner = None if owner_id is None else \
            WorldObject.get_object_by_id(owner_id)
        ...
        if self.has_running_costs:
            db_data = db("SELECT ticks FROM remaining_ticks_of_month WHERE "
                         "rowid=?", worldid)
            if not db_data:
                # this can happen when running costs are set when there
                # were no before we shouldn't crash because of changes
                # in yaml code, still it's suspicious
                self.log.warning('Object %s of type %s does not know when to'
                                 ' pay its rent.\nDisregard this when loading'
                                 ' old savegames or on running cost changes.',
        ...
        # island.add_building handles registration of building
        # for island and settlement
    ...
    def load_location(self, db, worldid):
        ...
        if isinstance(location_obj, Settlement):
            # workaround: island can't be fetched from world,
            # because it isn't fully constructed
    ...
    def get_buildings_in_range(self):
        ...
        for building in buildings:
            if building is self:
                continue
            if distances.distance_rect_rect(self.position,
                                            building.position) <= self.radius:
                yield building

    def update_action_set_level(self, level=0):
        """Updates this buildings action_set to a random actionset from
        the specified level (if an action set exists in that level).
        Its difference to get_random_action_set is that it just checks
        one level, and doesn't search for an action set everywhere,
        which makes it a lot more effective if you're just updating.
    ...
    def getInstance(cls, session, x, y, action='idle', level=0, rotation=45,
        ...
        @param rotation: rotation of the object.
                         Any of [45 + 90*i for i in xrange(0, 4)]
        ...
        # different rotations, and works with all quadratic buildings
        # (tested up to 4x4) for the first unquadratic building (2x4),
        # a hack fix was put into it. The plan for fixing this code in
        # general is to wait until there are more unquadratic buildings,
        # and figure out a pattern of the placement error,
