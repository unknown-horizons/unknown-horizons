...
from horizons.world.status import SettlerUnhappyStatus, \
    SettlerNotConnectedStatus
from horizons.world.production.producer import Producer
from horizons.messaging import AddStatusIcon, RemoveStatusIcon, \
    SettlerUpdate, SettlerInhabitantsChanged, UpgradePermissionsChanged


class SettlerRuin(BasicBuilding, BuildableSingle):
    """Building that appears when a settler got unhappy.
    The building does nothing.

    NOTE: Inheriting from BuildableSingle is necessary,
    cause it's built via Build Command, which
...
class Settler(BuildableRect, BuildingResourceHandler, BasicBuilding):
    """Represents a settlers house, that uses resources and creates
    inhabitants."""
    ...
    def __init__(self, x, y, owner, instance=None, **kwargs):
        kwargs['level'] = self.__class__.default_level_on_build
        # settlers always start in first level
        super(Settler, self).__init__(x=x, y=y, owner=owner,
    ...
    def initialize(self):
        ...
        if happiness is not None:
            self.get_component(StorageComponent).inventory.alter(RES.HAPPINESS,
                                                                 happiness)
        if self.has_status_icon:
            self.get_component(StorageComponent).inventory.add_change_listener(
                self._update_status_icon)
        # give the user a month (about 30 seconds) to build
        # a main square in range
        if self.owner.is_local_player:
            Scheduler().add_new_object(self._check_main_square_in_range,
                                       self, Scheduler().get_ticks_of_month(),
                                       loops=-1)
        self.__init()
        self.run()
    ...
    def load(self, db, worldid):
        ...
        self.inhabitants, last_tax_payed = \
            db("SELECT inhabitants, last_tax_payed FROM settler WHERE rowid=?",
               worldid)[0]
        remaining_ticks = \
            db("SELECT ticks FROM remaining_ticks_of_month WHERE rowid=?",
               worldid)[0][0]
    ...
    def _load_upgrade_data(self, db):
        ...
        upgrade_material_prodline = SettlerUpgradeData.get_production_line_id(
            self.level + 1)
        if not self.get_component(Producer).has_production_line(
                upgrade_material_prodline):
            return

        self._upgrade_production = self.get_component(Producer)\
            ._get_production(upgrade_material_prodline)

        # readd the res we already had, they can't be loaded since
        # storage slot limits for the special resources aren't saved
        resources = {}
        for resource, amount in db.get_storage_rowids_by_ownerid(self.worldid):
            resources[resource] = amount

        for res, amount in self._upgrade_production.get_consumed_resources()\
                .iteritems():
            # set limits to what we need
            self.get_component(StorageComponent).inventory.add_resource_slot(
                res, abs(amount))
            if res in resources:
                self.get_component(StorageComponent).inventory.alter(
                    res, resources[res])

        self._upgrade_production.add_production_finished_listener(
            self.level_up)
        self.log.debug("%s: Waiting for material to upgrade from %s",
                       self, self.level)
    ...
    def _add_upgrade_production_line(self):
        ...
        self._upgrade_production = self.get_component(
            Producer).add_production_by_id(
                upgrade_material_prodline)
        self._upgrade_production.add_production_finished_listener(
            self.level_up)

        # drive the car out of the garage to make space
    ...
    def upgrade_allowed(self):
        return self.session.world.get_settlement(
            self.position.origin).upgrade_permissions[self.level]
    ...
    def _on_change_upgrade_permissions(self, message):
        ...
        if production is not None:
            if production.is_paused() == self.upgrade_allowed:
                ToggleActive(self.get_component(Producer),
                             production).execute(self.session, True)

    @property
    def happiness(self):
        difficulty = self.owner.difficulty
        result = int(round(difficulty.extra_happiness_constant +
                           self.get_component(StorageComponent)
                           .inventory[RES.HAPPINESS] *
                           difficulty.happiness_multiplier))
        return max(0, min(result, self.get_component(
            StorageComponent).inventory.get_limit(RES.HAPPINESS)))
    ...
    def _update_level_data(self, loading=False, initial=False):
        ...
        self.inhabitants_max = self.session.db.get_tier_inhabitants_max(
            self.level)
        self.inhabitants_min = self.session.db.get_tier_inhabitants_min(
            self.level)
        # TODO This crops inhabitants at level down,
        #  but when can they exceed the limit?
        if self.inhabitants > self.inhabitants_max:
            self.inhabitants = self.inhabitants_max

        # consumption:
        # Settler productions are specified to be disabled by default
        # in the db, so we can enable
        # them here per level. Production data is save/loaded,
        ...
        if not loading:
            ...
            for prod_line in prod_comp.get_production_lines_by_level(
                    self.level):
                if not prod_comp.has_production_line(prod_line):
                    prod_comp.add_production_by_id(prod_line)
                # cross out the new lines from the current lines,
                #  so only the old ones remain
                if prod_line in current_lines:
                    current_lines.remove(prod_line)
            for line in current_lines[:]:  # iterate over copy for safe removal
                # all lines, that were added here but are not used due
                # to the current level
                # NOTE: this contains the upgrade material production line
                prod_comp.remove_production_by_id(line)
                # Make sure to set _upgrade_production to None in case
                #  we are removing it
                if (self._upgrade_production is not None and line ==
                        self._upgrade_production.get_production_line_id()):
    ...
    def run(self, remaining_ticks=None):
        ...
        Scheduler().add_new_object(self._tick, self, run_in=run_in,
    ...
    def pay_tax(self):
        ...
        # the money comes from nowhere, settlers seem to have an
        ...
        inhabitants_tax_modifier = float(
            self.inhabitants) / self.inhabitants_max
        taxes = self.tax_base * self.settlement.tax_settings[
            self.level] * happiness_tax_modifier * inhabitants_tax_modifier
        real_taxes = int(round(taxes * self.owner.difficulty.tax_multiplier))

        self.settlement.owner.get_component(
            StorageComponent).inventory.alter(RES.GOLD, real_taxes)
        self.last_tax_payed = real_taxes

        # decrease happiness http://wiki.unknown-horizons.org/w/
        # Settler_taxing#Formulae
        ...
        # NOTE: this formula was actually designed for a different use
        # case, where the happiness is calculated from the number of
        # available goods -/+ a certain tax factor. to simulate the
        # more dynamic, currently implemented approach (where every
        # event changes the happiness),
        # we simulate discontent of taxes by this:
        happiness_decrease -= 6
        self.get_component(StorageComponent).inventory.alter(
            RES.HAPPINESS, happiness_decrease)
        self._changed()
        self.log.debug("%s: pays %s taxes, -happy: %s new happiness: %s",
                       self, real_taxes,
                       happiness_decrease, self.happiness)

    def inhabitant_check(self):
        """Checks whether or not the population of this settler should
        ...
        if self.happiness > happy and self.inhabitants < self.inhabitants_max:
            change = 1
            self.log.debug("%s: inhabitants increase to %s", self,
                           self.inhabitants)
        elif self.happiness < sad and self.inhabitants > 1:
            change = -1
            self.log.debug("%s: inhabitants decrease to %s", self,
                           self.inhabitants)

        if change != 0:
            # see http://wiki.unknown-horizons.org/w/
            # Supply_citizens_with_resources
    ...
    def can_level_up(self):
        return self.happiness > self.__get_data(
            "happiness_level_up_requirement") and \
            self.inhabitants >= self.inhabitants_min and not \
            self._has_disaster()
    ...
    def level_check(self):
        ...
        if self.can_level_up():
                ...
                if self.owner.max_tier_notification < self.level_max:
                    if self.owner.is_local_player:
                        self.session.ingame_gui.message_widget.add(
                            point=self.position.center,
                            string_id='MAX_TIER_REACHED')
            ...
            if not self.upgrade_allowed:
                ToggleActive(self.get_component(Producer),
                             self._upgrade_production).execute(self.session,
                                                               True)

        elif self.happiness < self.__get_data(
                "happiness_level_down_limit") or \
                self.inhabitants < self.inhabitants_min:
            self.level_down()
            self._changed()

    def level_up(self, production=None):
        """Actually level up (usually called when the upgrade material
        has arrived)"""

        self._upgrade_production = None

        # just level up later that tick, it could disturb other code
        ...
        def _do_level_up():
            ...
            new_happiness = self.__get_data(
                "happiness_init_value") - self.happiness
            self.get_component(StorageComponent).inventory.alter(
                RES.HAPPINESS, new_happiness)
            self._changed()
    ...
    def level_down(self):
        if self.level == TIER.LOWEST:
            ...
            if self.owner.is_local_player:
                # check_duplicate: only trigger once for different settlers
                #  of a neighborhood
                self.session.ingame_gui.message_widget.add(
                    point=self.position.center,
    ...
    def make_ruin(self):
        ...
        command = Build(BUILDINGS.SETTLER_RUIN, self.position.origin.x,
                        self.position.origin.y, island=self.island,
    ...
    def _check_main_square_in_range(self):
        """Notifies the user via a message in case there is no main
        square in range"""
        ...
        for building in self.get_buildings_in_range():
            if building.id == BUILDINGS.MAIN_SQUARE:
                if StaticPather.get_path_on_roads(
                        self.island, self, building) is not None:
                    # a main square is in range
                    if hasattr(self, "_main_square_status_icon"):
                        RemoveStatusIcon.broadcast(
                            self, self, SettlerNotConnectedStatus)
                        del self._main_square_status_icon
                    return
        if not hasattr(self, "_main_square_status_icon"):
            self._main_square_status_icon = SettlerNotConnectedStatus(self)
            # save ref for removal later
    ...
    def _update_status_icon(self):
        if self.has_status_icon:
            unhappy = self.happiness < self.__get_data(
                "happiness_inhabitants_decrease_limit")
            # check for changes
            if unhappy and not hasattr(self, "_settler_status_icon"):
                self._settler_status_icon = SettlerUnhappyStatus(self)
                # save ref for removal later
    ...
    def __str__(self):
        try:
            return "{0!s}(l:{1!s};ihab:{2!s};hap:{3!s})".format(
                super(Settler, self).__str__(), self.level, self.inhabitants,
                self.happiness)
        except AttributeError:  # an attribute hasn't been set up
            return super(Settler, self).__str__()

    # @decorators.cachedmethod
    # TODO: replace this with a version that doesn't leak
    def __get_data(self, key):
        """Returns constant settler-related data from the db.
        The values are cached by python, so the underlying data
        must not change."""
        return int(
            self.session.db("SELECT value FROM balance_values WHERE name = ?",
                            key)[0][0])


class SettlerUpgradeData(object):
    """This is used as glue between the old upgrade system based on
    sqlite data used in a non-component environment
    and the current component version with data in yaml"""

    # basically, this is arbitrary as long as it's not the same as any
    # of the regular production lines of the settler. We reuse data
    # that has arbitrarily been set earlier to preserve savegame
    # compatibility.
    ...
    def get_production_lines(self):
        d = {}
        for level, prod_line_id in \
                self.__class__.production_line_ids.iteritems():
