...
def save_map(world, path, prefix):
    ...
    for island in world.islands:
        island_name = '{0!s}_island_{1:d}_{2:d}.sqlite'.format(
            prefix, island.origin.x, island.origin.y)
...
def add_resource_deposits(world, resource_multiplier):
    ...
    * the probability of choosing a resource deposit location
    ...
    def get_valid_locations(usable_part, island, width, height):
        """Return a list of all valid locations for a width times height object
        in the format [(value, (x, y), island), ...]."""
        locations = []
...
def add_nature_objects(world, natural_resource_multiplier):
    ...
    @param natural_resource_multiplier: multiply the amount of fish deposits,
    clay deposits, and mountains by this.
    """
