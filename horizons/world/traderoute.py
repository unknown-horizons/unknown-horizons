...
class TradeRoute(ChangeListener):
    ...
    def __init__(self, ship):
        ...
        self.wait_at_load = False
        # wait until every res has been loaded
        self.wait_at_unload = False
        # wait until every res could be unloaded

        self.current_transfer = {}
        # used for partial unloading in combination with waiting
    ...
    def remove_waypoint(self, position):
        ...
        try:
            ...
            # Usually caused by multiple clicks in short succession
            # with mp delay.
            pass
    ...
    def on_route_warehouse_reached(self):
        """Transfer resources, wait if necessary and move
        to next warehouse when possible"""
        warehouse = self.get_location()['warehouse']
        resource_list = self.current_transfer or self.get_location()[
            'resource_list']
        suppress_messages = self.current_transfer is not None
        # no messages from second try on
        ...
        if self.current_transfer is not None:
            for res in copy.copy(self.current_transfer):
                ...
                if self.current_transfer[res] == 0 or res not in\
                        self.get_location()['resource_list'] or \
                        cmp(self.current_transfer[res], 0) != \
                        cmp(self.get_location()['resource_list'][res], 0):
        ...
        if (not status.settlement_has_enough_space_to_take_res and
                self.wait_at_unload) or (
                    not status.settlement_provides_enough_res and
                    self.wait_at_load):
            self.current_transfer = status.remaining_transfers
            # retry
            Scheduler().add_new_object(self.on_route_warehouse_reached,
    ...
    def _transfer_resources(self, settlement, resource_list,
        ...
        class TransferStatus(object):
            def __init__(self):
                self.settlement_provides_enough_res = \
                    self.settlement_has_enough_space_to_take_res = True
        ...
        for res in resource_list:
            ...
            settlement_inv = settlement.get_component(
                StorageComponent).inventory
            if amount > 0:
                # Case A: Load from settlement onto ship.
                if settlement.owner is self.ship.owner:
                    # Check whether route asks for more of a resource
                    # than the settlement can offer currently.
                    # If so, only load that remainder instead.
                    if settlement_inv[res] < amount:
                        amount = settlement_inv[res]
                    # If due to previous trading there is a certain
                    # amount of this resource still left on the ship,
                    # don't overload: The ship only picks up enough
                    # to reach the amount defined in the route config
                    # in that case.
                    if ship_inv[res] + amount > self.get_location()[
                            'resource_list'][res]:
                        amount = self.get_location()[
                            'resource_list'][res] - ship_inv[res]
                    # The check if our ship has enough space
                    #  is handled implicitly below.
                    amount_transferred = settlement.transfer_to_storageholder(
                        amount, res, self.ship)
                else:
                    amount_transferred, error = settlement.get_component(
                        TradePostComponent).sell_resource(
                        self.ship.worldid, res, amount, add_error_type=True,
                        suppress_messages=suppress_messages)
                    if error == TRADE_ERROR_TYPE.PERMANENT:
                        # pretend to have everything and move on,
                ...
                if settlement.owner is self.ship.owner:
                    # Check if route asks for more of a resource than
                    # there is on the ship.
                    # If not, only move the remainder that we have
                    # loaded instead.
                    if ship_inv[res] < amount:
                        amount = ship_inv[res]
                    # Check if we can store everything we want
                    # to unload from ship.
                    # If not, only move the amount that can still
                    # be stored in settlement.
                    if settlement_inv.get_free_space_for(res) < amount:
                        amount = settlement_inv.get_free_space_for(res)

                    amount_transferred = self.ship.transfer_to_storageholder(
                        amount, res, settlement)
                else:
                    amount_transferred, error = settlement.get_component(
                        TradePostComponent).buy_resource(
                        self.ship.worldid, res, amount, add_error_type=True,
                        suppress_messages=suppress_messages)
                    if error == TRADE_ERROR_TYPE.PERMANENT:
    ...
    def move_to_next_route_warehouse(self, advance_waypoint=True):
        ...
        if self.ship.position.distance(warehouse.position.center) <=\
                self.ship.radius:
    ...
    def has_route(cls, db, worldid):
        """Check if a savegame contains route information
        for a certain ship"""
        return len(db("SELECT * FROM ship_route WHERE ship_id = ?",
                      worldid)) != 0
    ...
    def load(self, db):
        ...
        query = "SELECT warehouse_id FROM ship_route_waypoint WHERE " \
                "ship_id = ? ORDER BY waypoint_index"
        offices_id = db(query, self.ship.worldid)

        for office_id, in offices_id:
            warehouse = WorldObject.get_object_by_id(office_id)
            query = "SELECT res, amount FROM ship_route_resources WHERE " \
                    "ship_id = ? and waypoint_index = ?"
            resource_list = dict(db(query, self.ship.worldid,
                                    len(self.waypoints)))
        ...
        for res, amount in db("SELECT res, amount FROM "
                              "ship_route_current_transfer WHERE ship_id = ?",
                              self.ship.worldid):
            waiting = True
    ...
    def save(self, db):
        ...
        db("INSERT INTO ship_route(ship_id, enabled, current_waypoint,"
           ...
           self.wait_at_load, self.wait_at_unload)

        if self.current_transfer:
            for res, amount in self.current_transfer.iteritems():
                db("INSERT INTO ship_route_current_transfer(ship_id, res, "
                   "amount) VALUES(?, ?, ?)",
                   worldid, res, amount)

        for entry in self.waypoints:
            index = self.waypoints.index(entry)
            db("INSERT INTO ship_route_waypoint(ship_id, warehouse_id, "
               "waypoint_index) VALUES(?, ?, ?)",
               worldid, entry['warehouse'].worldid, index)
            for res in entry['resource_list']:
                db("INSERT INTO ship_route_resources(ship_id, waypoint_index, "
                   "res, amount) VALUES(?, ?, ?, ?)",
    ...
    def get_ship_status(self):
        ...
        if self.ship.is_moving():
            location = self.ship.get_location_based_status(
                self.ship.get_move_target())
            status_msg = _('Trade route: going to {location}').format(
                location=location)
            return (status_msg, self.ship.get_move_target())
        else:
            position = self.ship.get_location_based_status(self.ship.position)
            status_msg = _('Trade route: waiting at {position}').format(
                position=position)
