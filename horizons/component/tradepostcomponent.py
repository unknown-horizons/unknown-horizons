...
class TRADE_ERROR_TYPE(object):
    """Machine controlled entities need to know the difference.
    On this basis, they decide whether to retry the trade in a few seconds.
    """
...
class TradePostComponent(ChangeListener, Component):
    """This Class has to be inherited by every class that wishes
    to use BuySellTab and trade with the free trader.
    """
    ...
    def initialize(self):
        ...
        self.buy_list = {}
        # dict of resources that are to be bought. {resource_id: slot_id, ...}
        self.sell_list = {}
        # dict of resources that are to be sold.  {resource_id: slot_id, ...}
        self.trade_history = []
        # [(tick, player_id, resource_id, amount, gold), ...]
        # ordered by tick, player_id
        ...
        for slot_id in xrange(len(self.slots)):
            if self.slots[slot_id] is not None:
                db("INSERT INTO trade_slots(trade_post, slot_id, resource_id,"
                   ...
                   self.instance.worldid, slot_id,
                   self.slots[slot_id].resource_id,
                   self.slots[slot_id].selling, self.slots[slot_id].limit)

        db("INSERT INTO trade_values(object, total_income, total_expenses) "
           "VALUES (?, ?, ?)",
           self.instance.worldid, self.total_income, self.total_expenses)

        for row in self.trade_history:
            translated_tick = row[0] - Scheduler().cur_tick
            # pre-translate for the loading process
            db("INSERT INTO trade_history(settlement, tick, player, "
               "resource_id, amount, gold) VALUES(?, ?, ?, ?, ?, ?)",
               self.instance.worldid, translated_tick,
    ...
    def load(self, db, worldid):
        ...
        for (slot_id, resource_id, selling, limit) in db(
                "SELECT slot_id, resource_id, selling, trade_limit FROM "
                "trade_slots WHERE trade_post = ?", worldid):
            self.set_slot(slot_id, resource_id, selling, limit)

        self.total_income, self.total_expenses = db(
            "SELECT total_income, total_expenses"
            " FROM trade_values WHERE object = ?", worldid)[0]

        for row in db("SELECT tick, player, resource_id, amount, gold FROM "
                      "trade_history WHERE settlement = ? ORDER BY tick, "
                      "player", worldid):
    ...
    def buy(self, res, amount, price, player_id):
        ...
        if (res not in self.buy_list or
                ...
                amount + self.get_inventory()[res] >
                self.slots[self.buy_list[res]].limit):
            self._changed()
            return False
            ...
            assert remnant == 0
            self.trade_history.append((Scheduler().cur_tick, player_id, res,
                                       amount, -price))
    ...
    def sell(self, res, amount, price, player_id):
        ...
        if (res not in self.sell_list or
                ...
                self.get_inventory()[res] - amount <
                self.slots[self.sell_list[res]].limit):
            self._changed()
            return False
    ...
    def sell_resource(self, ship_worldid, resource_id, amount,
                      add_error_type=False, suppress_messages=False):
        """ Attempt to sell the given amount of resource to the ship,
        returns the amount sold.
        @param add_error_type: if True, return tuple where second item
                              is ERROR_TYPE"""
        ...
        if resource_id not in self.sell_list:
            return err(_("The trade partner does not sell this."),
                       TRADE_ERROR_TYPE.PERMANENT)

        price = int(self.session.db.get_res_value(resource_id) *
                    TRADER.PRICE_MODIFIER_BUY)
        # price per ton of resource
        assert price > 0

        # can't sell more than the ship can fit in its inventory
        amount = min(amount, ship.get_component(StorageComponent).inventory.
                     get_free_space_for(resource_id))
        if amount <= 0:
            return err(_("You can not store this."),
                       TRADE_ERROR_TYPE.PERMANENT)
        # can't sell more than the ship's owner can afford
        amount = min(amount, ship.owner.get_component(StorageComponent).
                     inventory[RES.GOLD] // price)
        if amount <= 0:
            return err(_("You can not afford to buy this."),
        ...
        if amount <= 0:
            return err(_("The trade partner does not sell more of this."),
                       TRADE_ERROR_TYPE.TEMPORARY)
        ...
        self.sell_history[Scheduler().cur_tick] = (resource_id, amount,
                                                   total_price)
        self.total_income += total_price
        self._changed()
        return amount if not add_error_type else amount, \
            TRADE_ERROR_TYPE.NO_ERROR
    ...
    def buy_resource(self, ship_worldid, resource_id, amount,
        ...
        if resource_id not in self.buy_list:
            return err(_("The trade partner does not buy this."),
                       TRADE_ERROR_TYPE.PERMANENT)
        ...
        if amount <= 0:
            return err(_("You do not possess this."),
                       TRADE_ERROR_TYPE.PERMANENT)
        # can't buy more than we can fit in the inventory
        amount = min(amount, self.get_inventory().get_free_space_for(
            resource_id))
        if amount <= 0:
            return err(_("The trade partner can not store more of this."),
                       TRADE_ERROR_TYPE.TEMPORARY)
        # can't buy more than we can afford
        amount = min(amount, self.get_owner_inventory()[RES.GOLD] // price)
        if amount <= 0:
            return err(_("The trade partner can not afford to buy this."),
                       TRADE_ERROR_TYPE.TEMPORARY)

        # can't buy more than we are trying to buy according to the settings
        amount = min(amount, self.slots[self.buy_list[
            resource_id]].limit - self.get_inventory()[resource_id])
        if amount <= 0:
            return err(_("The trade partner does not buy more of this."),
                       TRADE_ERROR_TYPE.TEMPORARY)
    ...
    def sell_income(self):
        ...
        income = 0
        last_month_start = Scheduler().cur_tick - Scheduler(
            ).get_ticks_of_month()
        keys_to_delete = []
