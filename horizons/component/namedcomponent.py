...
class NamedComponent(Component):
    """An object that has a special name. "Special" means,
    that it's not (only) autogenerated."""
    ...
    def get_default_name(self):
        ...
        if available_names != []:
            ...
            index = 2
            while u"{newname} {index}".format(
                    newname=newname, index=index) in NamedComponent.names_used:
                index += 1
    ...
    def save(self, db):
        ...
        db("INSERT INTO name (rowid, name) VALUES(?, ?)",
           self.instance.worldid, self.name)
...
class ShipNameComponent(NamedComponent):
    ...
    def _possible_names(self):
        names = self.session.db("SELECT name FROM shipnames WHERE "
                                "for_player = 1")
...
class SettlementNameComponent(NamedComponent):
    ...
    def _possible_names(self):
        names = self.session.db("SELECT name FROM citynames WHERE "
                                "for_player = 1")
        return [x[0] for x in names]
...
class SoldierNameComponent(NamedComponent):
    ...
    def _possible_names(self):
        names = self.session.db("SELECT name FROM groundunitnames WHERE "
                                "for_soldier = 1")
        return [x[0] for x in names]
