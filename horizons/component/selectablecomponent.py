...
class SelectableComponent(Component):
    ...
    Has to be subdivided in buildings and units,
    which is further specialized to ships.
    ...
    def get_instance(cls, arguments):
        ...
        # it would be parsed
        TYPES = {
            'building': SelectableBuildingComponent,
    ...
    def __init__(self, tabs, enemy_tabs, active_tab=None):
        ...
        self.active_tab = resolve_tab(
            active_tab) if active_tab is not None else None
    ...
    def show_menu(self, jump_to_tabclass=None):
        ...
        @param jump_to_tabclass: open the first tab that
                                 is a subclass to this parameter
        """
        from horizons.gui.tabs import TabWidget
        tablist = None
        if self.instance.owner is not None and \
        ...
        tabclasses = [tabclass for tabclass in tablist if tabclass.shown_for(
            self.instance)]
        try:
...
class SelectableBuildingComponent(SelectableComponent):
    ...
    # these smell like instance attributes, but sometimes have to be used in
    # non-instance contexts (e.g. building tool).
    ...
    # tiles that are selected. used for clean deselect.
    ...
    def __init__(self, tabs, enemy_tabs, active_tab=None,
        ...
        super(SelectableBuildingComponent, self).__init__(
            tabs, enemy_tabs, active_tab=active_tab)
        self.range_applies_only_on_island = range_applies_only_on_island

    def initialize(self):
        # check for related buildings (defined in db, not yaml)
        related_buildings = self.session.db.get_related_building_ids_for_menu(
            self.instance.id)
    ...
    def deselect(self):
        ...
        if self._selected:
            ...
            for fake_tile in self.__class__._selected_fake_tiles.l:
                self.session.view.layers[LAYERS.FIELDS].deleteInstance(
                    fake_tile)
    ...
    def select_building(cls, session, position, settlement,
        ...
        Only works on a subclass of BuildingClass,
        since it requires certain class attributes.
    ...
    def deselect_building(cls, session):
        ...
        @return: list of tiles that were deselected
                 (only normal tiles, no fake tiles)"""
        remove_colored = session.view.renderer[
            'InstanceRenderer'].removeColored
    ...
    def select_many(cls, buildings, renderer):
        ...
        # they cannot share tiles, and we can then just access
        # the settlements ground map
        buildings_sorted = sorted(buildings,
                                  key=operator.attrgetter('settlement'))
        for settlement, buildings in itertools.groupby(
            ...
            coords = set(coord for building in buildings for coord in
                         building.position.get_radius_coordinates(
                             building.radius, include_self=True))

            for coord in coords:
                tile = settlement.ground_map.get(coord)
                if tile:
                    if ('constructible' in tile.classes or 'coastline' in
                            tile.classes):
    ...
    def _do_select(cls, renderer, position, world, settlement,
        ...
        if range_applies_only_on_island:
            ground_holder = None
            # use settlement or island as tile provider (prefer settlement,
            # since it contains fewer tiles)
            if settlement is None:
                ground_holder = island
            else:
                ground_holder = settlement

            for tile in ground_holder.get_tiles_in_radius(position, radius,
                                                          include_self=False):
                if 'constructible' in tile.classes or 'coastline' in \
                        tile.classes:
                    if settlement is None and tile.settlement is not None:
                        # trying to build a warehouse and the tile is already
    ...
    def _add_selected_tile(cls, tile, renderer, remember=True):
        """
        @param remember: whether to keep track of this tile.
                         Set to False on recolorings.
        """
        if remember:
...
class SelectableUnitComponent(SelectableComponent):
    ...
    def select(self, reset_cam=False):
        ...
        self.session.view.renderer['InstanceRenderer'].addOutlined(
    ...
    def deselect(self):
        ...
        self.session.view.renderer['InstanceRenderer'].removeOutlined(
            self.instance._instance)
...
class SelectableFisherComponent(SelectableBuildingComponent):
    ...
    def _do_select(cls, renderer, position, world, settlement, radius,
        ...
        cls._init_fake_tile()
        layer = world.session.view.layers[LAYERS.FIELDS]
        for fish_deposit in world.get_providers_in_range(
                RadiusRect(position, radius), res=RES.FISH):
