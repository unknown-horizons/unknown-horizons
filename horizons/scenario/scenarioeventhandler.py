...
class ScenarioEventHandler(LivingObject):
	...
	def to_yaml(self):
		...
		yaml_code += ', events: [ {0!s} ] }}'.format(', '.join(event.to_yaml() for event in self._events))
		return yaml_code
...
class _Action(object):
	...
	def __init__(self, action_dict):
		...
		try:
			...
			raise InvalidScenarioFileFormat('Found invalid action type: {0!s}'.format(self.action_type))
...
class _Condition(object):
	...
	def __init__(self, session, cond_dict):
		...
		try:
			...
			raise InvalidScenarioFileFormat('Found invalid condition type: {0!s}'.format(self.cond_type))
