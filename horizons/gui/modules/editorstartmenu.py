...
class EditorStartMenu(Window):
    ...
    def show(self):
        ...
        events['create_new_map'] = Callback(self._select_mode,
                                            'create_new_map')
        events['load_existing_map'] = Callback(self._select_mode,
                                               'load_existing_map')
        events['load_saved_game_map'] = Callback(self._select_mode,
                                                 'load_saved_game_map')
    ...
    def _select_mode(self, mode):
        ...
        self._right_side = modes[mode](self._windows,
                                       self._gui.findChild(name='right_side'))
        self._right_side.show()
        self._gui.show()
...
class EditorCreateMapWidget(object):
    ...
    def __init__(self, windows, parent_widget):
        ...
        for size in self.sizes:
            option_name = 'size_{0:d}'.format(size)
            # size of empty map in map editor
            self._gui.findChild(name=option_name).text = \
                _('{size}x{size} tiles').format(size=size)
    ...
    def act(self):
        for size in self.sizes:
            ...
            if self._gui.findChild(name=option_name).marked:
                ...
                # the empty list is interpreted as the empty list
                # of random map island strings
