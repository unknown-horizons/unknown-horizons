...
from horizons.command.diplomacy import AddAllyPair, AddNeutralPair,\
    AddEnemyPair
...
class PlayerDiplomacyTab(TabInterface):
    ...
    It displays the menu for selecting the status between
    the local player and the tab's player
    """
    ...
    def show(self):
        ...
        self.diplomacy.add_diplomacy_status_changed_listener(
            Callback(self.check_diplomacy_state))

    def hide(self):
        super(PlayerDiplomacyTab, self).hide()
        self.diplomacy.remove_diplomacy_status_changed_listener(
            Callback(self.check_diplomacy_state))

    def add_ally(self):
        """
        Callback for setting ally status between local player
        and tab's player
        """
        AddAllyPair(self.player, self.local_player).execute(
            self.player.session)
    ...
    def add_neutral(self):
        """
        Callback for setting neutral status between local player
        and tab's player
        """
        AddNeutralPair(self.player, self.local_player).execute(
            self.player.session)
        # check the correct checkbox
        self.check_diplomacy_state()
    ...
    def add_enemy(self):
        """
        Callback for setting enemy status between local player
        and tab's player
        """
        AddEnemyPair(self.player, self.local_player).execute(
            self.player.session)
        # check the correct checkbox
        self.check_diplomacy_state()
    ...
    def check_diplomacy_state(self):
        """
        Checks the box with the diplomacy status between local player
        and selected player
        """
        # Uncheck all boxes.
        ...
        self.widget.distributeData({'{0!s}_check_box'.format(state): True})
...
class DiplomacyTab(TabWidget):
    ...
    def __init__(self, ingame_gui, world):
        ...
        super(DiplomacyTab, self).__init__(ingame_gui, tabs=tabs,
                                           name="diplomacy_widget")

    @classmethod
    def is_useable(cls, world):
        """Diplomacy only makes sense if there is another player.
        Pirates do not qualify as players: right now they're not
