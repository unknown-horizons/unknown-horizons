...
class SelectMultiTab(TabInterface):
    ...
    def add_entry(self, entry):
        ...
        self.widget.findChild(name="hbox_{0!s}".format(self.row_number)).addChild(
            entry.widget)
...
class UnitEntry(object):
    def __init__(self, instances, show_number=True):
        ...
        if show_number:
            self.widget.findChild(name="instance_number").text = unicode(
                len(self.instances))
        # only two callbacks are needed so drop unwanted changelistener
        #  inheritance
        for i in instances:
    ...
    def remove(self):
        ...
        """
        for instance in self.instances:
            instance.discard_remove_listener(Callback(
                self.on_instance_removed, instance))
            health_component = instance.get_component(HealthComponent)
            if health_component.has_damage_dealt_listener(self.draw_health):
                health_component.remove_damage_dealt_listener(self.draw_health)
