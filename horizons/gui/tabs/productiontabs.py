...
class ProductionOverviewTab(OverviewTab):
    ...
    ARROW_TOP = "content/gui/icons/templates/production/" \
                "production_arrow_top.png"
    ARROW_MID = "content/gui/icons/templates/production/" \
                "production_arrow_start.png"
    ARROW_BOTTOM = "content/gui/icons/templates/production/" \
                   "production_arrow_bottom.png"
    ARROW_CONNECT_UP = "content/gui/icons/templates/production/" \
                       "production_arrow_connect_up.png"
    ARROW_CONNECT_DOWN = "content/gui/icons/templates/production/" \
                         "production_arrow_connect_down.png"
    ARROWHEAD_TOP = "content/gui/icons/templates/production/" \
                    "production_arrowhead_top.png"
    ARROWHEAD_MID = "content/gui/icons/templates/production/" \
                    "production_arrow_head.png"
    ARROWHEAD_BOTTOM = "content/gui/icons/templates/production/" \
                       "production_arrowhead_bottom.png"
    ARROWHEAD_CONNECT_UP = "content/gui/icons/templates/production/" \
                           "production_arrowhead_connect_up.png"
    ARROWHEAD_CONNECT_DOWN = \
        "content/gui/icons/templates/production/" \
        "production_arrowhead_connect_down.png"
    ICON_HEIGHT = ImageFillStatusButton.CELL_SIZE[1] + \
        ImageFillStatusButton.PADDING
    ...
    def get_displayed_productions(self):
        """List all possible productions of a buildings
        sorted by production line id.
    ...
    def refresh(self):
        ...
        # sort by production line id to have a consistent
        #  (basically arbitrary) order
        for production in self.get_displayed_productions():
            ...
            if production.is_paused():
                centered_container.removeChild(centered_container.findChild(
                ...
                if production.get_state() == PRODUCTION.STATES.producing:
                    ...
                    centered_container.removeChild(toggle_icon)
                    # fix z-ordering
                    ...
                    anim.start(1.0 / 12, -1)
                    # always start anew, people won't notice
            ...
            centered_container.mapEvents({
                'toggle_active': Callback(toggle_active.execute,
                                          self.instance.session)
            })
            # NOTE: this command causes a refresh, so we needn't change
            #  the toggle_active-button-image
    ...
    def _center_production_line(self, parent_container, production):
        ...
        @return: y value to center other gui parts (toggle icon etc.)
                 around vertically.
        """
        ...
        if input_amount != output_amount:
            ...
            output_container = parent_container.findChild(
                name='output_container')
            input_container.position = (input_container.position[0],
                                        center_y_in)
            output_container.position = (output_container.position[0],
                                         center_y_out)
        # Draw and combine arrows for input and output.
        if input_amount > 0:
            self._draw_pretty_arrows(parent_container, input_amount, x=58,
                                     y=center_y, out=False)
        if output_amount > 0:
            self._draw_pretty_arrows(parent_container, output_amount, x=96,
                                     y=center_y, out=True)
        return center_y

    def _draw_pretty_arrows(self, parent_container, amount, x=0, y=0,
                            out=False):
        ...
        for res in xrange(amount // 2):
            ...
            # Place a connector image (the | in above sketch) that
            # vertically connects the input resource arrows.
            # We need those if the production line has more
            # than three input resources. Connectors are placed
            # in the inner loop parts.
            place_connectors = amount > (3 + 2 * res)
            if place_connectors:
                # the connector downwards connects top_arrows
                if out:
                    down_connector = Icon(
                        image=self.__class__.ARROWHEAD_CONNECT_DOWN)
                else:
                    down_connector = Icon(
                        image=self.__class__.ARROW_CONNECT_DOWN)
                down_connector.position = (98, y - offset)
                parent_container.insertChild(down_connector, 0)
                # the connector upwards connects up_arrows
                if out:
                    up_connector = Icon(
                        image=self.__class__.ARROWHEAD_CONNECT_UP)
                else:
    ...
    def _refresh_utilization(self):
        ...
        if self.instance.has_component(Producer):
            utilization = int(round(self.instance.get_component(
                Producer).capacity_utilization * 100))
        self.widget.child_finder('capacity_utilization').text = unicode(
            utilization) + u'%'

    def _add_resource_icons(self, container, resources, marker=False):
        calculate_position = lambda amount: (
            amount * 100) // inventory.get_limit(res)
        for res in resources:
            ...
            image_button = ImageFillStatusButton.init_for_res(
                self.instance.session.db, res, inventory[res], filled,
                marker=marker_level, use_inactive_icon=False, uncached=True)
    ...
    def show(self):
        ...
        Scheduler().add_new_object(Callback(self._refresh_utilization),
                                   self, run_in=GAME_SPEED.TICKS_PER_SECOND,
                                   loops=-1)
...
class LumberjackOverviewTab(ProductionOverviewTab):
    ...
    def update_data(self):
        ...
        if enough_res and field_comp.how_many > 0:
            ...
            icon.image = "content/gui/images/buttons/" \
                         "buildmenu_button_bg_bw.png"
        ...
        click_cb = Callback.ChainedCallbacks(field_comp.fill_range,
                                             self.update_data)
        enter_cb = Callback(res_bar.set_construction_mode, self.instance,
                            field_comp.total_cost)
        # TODO the tooltip should actually hide on its own. Ticket #1096
        exit_cb = Callback.ChainedCallbacks(res_bar.close_construction_mode,
                                            button.hide_tooltip)
