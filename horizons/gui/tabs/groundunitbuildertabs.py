...
class UnitbuilderTabBase(ProducerOverviewTabBase):
	...
	def update_production_is_active_container(self, progress_container, container_active, cancel_container, production_lines):
		...
		container_active.findChild(name="headline_UB_groundunit_label").text = _(name)
	...
	def update_groundunit_icon(self, container_active, produced_unit_id):
		"""Update the icon displaying the groundunit that is being built."""
		groundunit_icon = container_active.findChild(name="UB_cur_groundunit_icon")
		groundunit_icon.helptext = self.instance.session.db.get_groundunit_tooltip(produced_unit_id)
		groundunit_icon.image = self.__class__.UNIT_PREVIEW_IMAGE.format(type_id=produced_unit_id)
...
class groundunitSoidersTab(BoatbuilderSelectTab):
