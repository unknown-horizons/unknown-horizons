...
class TabWidget(object):
	...
	def _init_tab_buttons(self):
		...
		for index, tab in enumerate(self._tabs):
			...
			container = Container(name="container_{0!s}".format(index))
			background = Icon(name="bg_{0!s}".format(index))
	...
	def show_tab(self, number):
		...
		old_bg = self.content.findChild(name = "bg_{0!s}".format(self._tabs.index(self.current_tab)))
		...
		new_bg = self.content.findChild(name = "bg_{0!s}".format(number))
