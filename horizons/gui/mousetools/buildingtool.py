...
from horizons.messaging import SettlementRangeChanged, WorldObjectDeleted, \
    SettlementInventoryUpdated, PlayerInventoryUpdated


class BuildingTool(NavigationTool):
    """Represents a dangling tool after a building was selected from the list.
    Builder visualizes if and why a building can not be built under
    the cursor position.
    @param building: selected building type"
    @param ship: If building from a ship, restrict to range of ship

    The building tool has been under heavy development for several years,
      it's a collection
    of random artifacts that used to have a purpose once.

    Terminology:
    - Related buildings: Buildings lower in the hierarchy, needed by current
      building to operate
    (tree when building lumberjack)
    - Inversely related building: lumberjack for tree. Need to show its range
      to place building,
    it must be in range.
    - Building instances/fife instances: the image of a building,
      that is dragged around.

    Main features:
    - Display tab to the right, showing build preview icon
      and rotation button (draw_gui(), load_gui())
    - Show buildable ground (highlight buildable) as well as ranges
      of inversely related buildings
        - This also is called for tiles that need to be recolored, other
          highlights sometimes draw over
          tiles, then this is called again to redo the original coloring.
    - Catch mouse events and handle preview on map:
        - Get tentative fife instances for buildings, draw them colored
          according to buildability
        - Check for resources missing for build
        - Making surrounding of preview transparent, so you see where
          you are building in a forest
        - Highlight related buildings, that are in range
        - Draw building range and highlight related buildings that
          are in range in this position
        (_color_preview_building)
    ...
    - Update, which is mainly controlled by preview_build
        - Update highlights related to build
            - Transparency
            - Inversely related buildings in range
              (highlight_related_buildings)
            - Related buildings in range (_color_preview_build)
        - Set new instances
        - During this time, don't touch anything set by highlight_buildable,
          or restore it later
    ...
    def __init__(self, session, building, ship=None, build_related=None):
        ...
        # list action set ids of list above
        self.buildings_fife_instances = {}
        # fife instances of possible builds
    ...
    def highlight_buildable(self, tiles_to_check=None, new_buildings=True):
        """Highlights all buildable tiles and select buildings that are
        inversely related in order to show their range.
        @param tiles_to_check: list of tiles to check for coloring.
        @param new_buildings: Set to True if you have set tiles_to_check
                              and there are new buildings.
        An internal structure for optimization will be amended."""
        ...
        related = self.session.db.get_inverse_related_building_ids(
            self._class.id)
        ...
        if self._class.id in \
            ...
            related = related + [self._class.id]
            # don't += on retrieved data from db
        ...
        if tiles_to_check is None or new_buildings:  # first run, check all
            ...
            tiles = self.selectable_comp.select_many(buildings_to_select,
                                                     renderer)
            ...
            for tile in tiles_to_check:
                ...
                if tile in self._related_buildings_selected_tiles:
                    self.selectable_comp._add_selected_tile(tile, renderer,
                                                            remember=False)
    ...
    def _color_buildable_tile(self, tile):
        self._buildable_tiles.add(tile)
        # it's a set, so duplicates are handled
    ...
    def _on_worldobject_deleted(self, message):
        ...
        self._highlighted_buildings = set(
            ...
            check_building(tup[0]))
        self._related_buildings = set(filter(check_building,
    ...
    def load_gui(self):
        ...
        self.__class__.gui.findChild(
            ...
            building=_(self._class.name))
        self.__class__.gui.findChild(name='running_costs').text = unicode(
    ...
    def draw_gui(self):
        ...
        for action_option in ['idle', 'idle_full', 'abcd']:
            ...
            # If no idle, idle_full or abcd animation found,
            # use the first you find
            action = action_sets[action_set].keys()[0]
        rotation = (self.rotation + int(
            self.session.view.cam.getRotation()) - 45) % 360
        image = sorted(action_sets[action_set][action][rotation].keys())[0]
        if GFX.USE_ATLASES:
            # Make sure the preview is loaded
            horizons.globals.fife.animationloader.load_image(image,
                                                             action_set,
                                                             action,
                                                             rotation)
        ...
        if width > max_width:
            ...
            building_icon.max_size = building_icon.min_size = \
                building_icon.size = size
    ...
    def preview_build(self, point1, point2, force=False):
        ...
        self.log.debug("BuildingTool: preview build at %s, %s", point1,
                       point2)
        new_buildings = self._class.check_build_line(self.session, point1,
                                                     point2,
                                                     rotation=self.rotation,
                                                     ship=self.ship)
        # optimization: If only one building is in the preview
        # and the position hasn't changed
        # => don't preview. Otherwise the preview is redrawn
        # on every mouse move
        ...
        for i, building in enumerate(self.buildings):
            ...
            # workaround for buildings like settler, that don't use
            # the current level of the player,
            # but always start at a certain lvl
            ...
            if self._class.id != BUILDINGS.WAREHOUSE:
                # Player shouldn't be allowed to build in this case, else it
                # can trigger a new_settlement notification
                if settlement is None:
                    building.buildable = False

            # check required resources
            (enough_res, missing_res) = Build.check_resources(
                needed_resources, self._class.costs,
            ...
            # draw ordinary ranges first, then later color related buildings
    ...
    def _draw_preview_building_range(self, building, settlement):
        ...
        if hasattr(self.selectable_comp, 'range_applies_only_on_island'):
            radius_only_on_island = \
                self.selectable_comp.range_applies_only_on_island

        self.selectable_comp.select_building(self.session,
                                             building.position,
                                             settlement,
                                             self._class.radius,
                                             radius_only_on_island)
    ...
    def _highlight_related_buildings_in_range(self, building, settlement):
        ...
        that are in range of the build preview"""
        if settlement is not None:
            ...
            for tile in settlement.get_tiles_in_radius(building.position,
                ...
                if (obj is not None) and (obj.id in related) and (
                    ...
                    self._highlighted_buildings.add((obj, False))
                    ...
                    self.renderer.addOutlined(inst,
                                              *self.related_building_outline)
                    self.renderer.addColored(inst,
                                             *self.related_building_color)

    def _make_surrounding_transparent(self, building):
        """Makes the surrounding of building_position transparent
        and hide buildings that are built upon (tearset)"""
        world_contains = self.session.world.map_dimensions.\
            contains_without_border
        ...
        for to_tear_worldid in building.tearset:
            ...
            inst.get2dGfxVisual().setTransparency(255)
            # full transparency = hidden
            self._transparencified_instances.add(weakref.ref(inst))

    def _highlight_inversely_related_buildings(self, building, settlement):
        """Point out buildings that are inversely related
        (e.g. lumberjacks when building trees). This is triggered on each
        preview change and highlights only those in range
        """
        # tuple for fast lookup with few elements
        ids = tuple(self.session.db.get_inverse_related_building_ids(
            self._class.id))
            ...
            if tile.object is not None and tile.object.id in ids:
                ...
                if building.position.distance((tile.x, tile.y)) <= \
                   Entities.buildings[related_building.id].radius:
                    ...
                    self._highlighted_buildings.add((related_building, True))
                    # True: was_selected, see _restore_highlighted_buildings
                    # currently same code as coloring normal related buildings
                    # (_color_preview_build())
    ...
    def _restore_highlighted_buildings(self):
        ...
        # assemble list of all tiles that have are occupied by
        # now restored buildings
        ...
        for (building, was_selected) in self._highlighted_buildings:
            ...
            if was_selected:
                self.renderer.addColored(
                    inst,
                    *self.selectable_comp.selection_color)
            self.renderer.removeOutlined(inst)
            modified_tiles.extend(
                (self.session.world.get_tile(point) for point in
                 building.position)
            )
    ...
    def mouseReleased(self, evt):
        ...
        if evt.isConsumedByWidgets():
            ...
            # check if position has changed with this event
            ...
            if building.buildable:
                ...
                for position in building.position:
                    ...
                    if tile in self._buildable_tiles:
                        # for some kind of buildabilities, not every coord
                        # of the building is buildable (e.g. fisher: only
                        # coastline is marked as buildable). For those tiles,
                ...
                self._remove_listeners()
                # Remove change listeners for update_preview
                ...
                if len(self.buildings) == 1:
                    # only give messages for single bulds
    ...
    def _remove_listeners(self):
        ...
        if self.last_change_listener is not None:
            self.last_change_listener.discard_change_listener(
                self.force_update)
            self.last_change_listener.discard_change_listener(
                self.highlight_buildable)
            self._build_logic.remove_change_listener(
    ...
    def _rotate(self, degrees):
        ...
        self.log.debug("BuildingTool: Building rotation now: {0}".format(
            self.rotation))
        self.update_preview()
        self.draw_gui()
    ...
    def _remove_building_instances(self):
        ...
        try:
            ...
            deselected_tiles = self.selectable_comp.deselect_building(
                self.session)
            # redraw buildables (removal of selection might have tampered
            # with it)
        ...
        for building in self._related_buildings:
            ...
            building.get_component(
                SelectableComponent).set_selection_outline()
    ...
    def _restore_transparencified_instances(self):
        ...
        for inst_weakref in self._transparencified_instances:
            ...
            if fife_instance:
                # remove transparency only if trees aren't supposed
                # to be transparent as default
                if not hasattr(fife_instance, "keep_translucency")\
    ...
    def _remove_coloring(self):
        """Removes coloring from tiles, that indicate that
...
class ShipBuildingToolLogic(object):
    """Helper class to separate the logic needed when building from a ship
    ...
    def highlight_buildable(self, building_tool, tiles_to_check=None):
        ...
        if tiles_to_check is not None:
            # only check these tiles (build from ship)
            for tile in tiles_to_check:
            ...
            for island in session.world.get_islands_in_radius(
                    self.ship.position, self.ship.radius):
                for tile in island.get_surrounding_tiles(self.ship.position,
                                                         self.ship.radius):
                    if is_tile_buildable(session, tile, self.ship):
...
class SettlementBuildingToolLogic(object):
    """Helper class to separate the logic needed when building from
    a settlement from the main building tool
    """
...
class BuildRelatedBuildingToolLogic(SettlementBuildingToolLogic):
    """Same as normal build, except quitting it drops
    to the build related tab.
    """
    ...
    def _reshow_tab(self):
        ...
        self.instance().get_component(SelectableComponent).show_menu(
            jump_to_tabclass=BuildRelatedTab)
