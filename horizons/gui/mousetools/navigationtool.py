...
class NavigationTool(CursorTool):
    ...
    # last received mouse event position, fife.ScreenPoint
    ...
    def __init__(self, session):
        ...
        class CoordsTooltip(object):
            ...
            def get_instance(cls, cursor_tool):
                if (cursor_tool.session.ingame_gui.coordinates_tooltip is not
                        None):
    ...
    def mouseDragged(self, evt):
        if evt.getButton() == fife.MouseEvent.MIDDLE:
            ...
            # Else the event will mistakenly be delegated if
            # the left mouse button is hit while
    ...
    def mouseMoved(self, evt):
        ...
        if mousepoint.x < VIEW.AUTOSCROLL_WIDTH:
            ...
            x += VIEW.AUTOSCROLL_WIDTH + mousepoint.x - \
                 self.session.view.cam.getViewPort().right()
        if mousepoint.y < VIEW.AUTOSCROLL_WIDTH:
            y -= VIEW.AUTOSCROLL_WIDTH - mousepoint.y
                             ...
                             VIEW.AUTOSCROLL_WIDTH):
            y += VIEW.AUTOSCROLL_WIDTH + mousepoint.y - \
                 self.session.view.cam.getViewPort().bottom()
        x *= 10
        y *= 10
    ...
    def onCommand(self, command):
        ...
        in fife.
        It's usually about mouse/keyboard focus or window
         being iconified/restored.
        """
        ...
        if command.getCommandType() in stop_scrolling_on:
            # it has been randomly observed twice that this code is reached
            # with session being None or partly deinitialized.
            #  Since it is unknown how fife handles this and why
    ...
    def _schedule_hover_instance_update(self):
        """Hover instances have potentially changed,
        do an update in a timely fashion
        (but not right away)"""
        if not self._hover_instances_update_scheduled:
            self._hover_instances_update_scheduled = True
            ExtScheduler().add_new_object(self._send_hover_instance_upate,
                                          self,
                                          run_in=self.__class__.
                                          HOVER_INSTANCES_UPDATE_DELAY)
