...
class MessageWidget(LivingObject):
    ...
    BG_IMAGE_MIDDLE = 'content/gui/images/background/widgets/' \
                      'message_bg_middle.png'
    IMG_HEIGHT = 24  # distance of above segments in px.
    LINE_HEIGHT = 17
    ICON_TEMPLATE = 'messagewidget_icon.xml'
    MSG_TEMPLATE = 'messagewidget_message.xml'
    CHARS_PER_LINE = 31
    # character count after which we start new line. no wrap
    ...
    def __init__(self, session):
        ...
        self.msgcount = itertools.count()
        # sort to preserve order after loading

        self.widget = load_uh_widget(self.ICON_TEMPLATE)
        screenheight = horizons.globals.fife.engine_settings.getScreenHeight()
        self.widget.position = (5, (screenheight // 2) -
                                (self.widget.size[1] // 2))

        self.text_widget = load_uh_widget(self.MSG_TEMPLATE)
        self.reference_text_widget_position = (
            self.widget.x + self.widget.width, self.widget.y)
        ...
        self._last_message = {}
        # used to detect fast subsequent messages in add()
        self.draw_widget()

    def add(self, string_id, point=None, msg_type=None, message_dict=None,
            play_sound=True, check_duplicate=False):
        """Adds a message to the MessageWidget.
        @param point: point where the action took place.
                      Clicks on the message will then focus that spot.
        @param id: message id string, needed to retrieve the message text
                   from the content database.
        @param msg_type: message type; determines what happens on click
        @param message_dict: dict with strings to replace in the message,
                             e.g. {'player': 'Arthus'}
        @param play_sound: whether to play the default message
                           speech for string_id
        @param check_duplicate: check for pseudo-duplicates
                                (similar messages recently nearby)
        """
        if check_duplicate:
            if string_id in self._last_message:
                when, where = self._last_message[string_id]
                if when > Scheduler().cur_tick - Scheduler().get_ticks(
                        self.__class__._DUPLICATE_TIME_THRESHOLD) and \
                        where.distance(point) < \
                        self.__class__._DUPLICATE_SPACE_THRESHOLD:
        ...
        return self._add_message(_IngameMessage(point=point, id=string_id,
                                                msg_type=msg_type,
                                                created=self.msgcount.next(),
    ...
    def add_custom(self, messagetext, point=None, msg_type=None,
                   visible_for=40, icon_id=1):
        """ See docstring for add().
        Uses no predefined message template from content database
        like add() does.n Instead, directly provides text and icon
        to be shown (messagetext, icon_id)
        @param visible_for: how many seconds the message
                            will stay visible in the widget
        """
        return self._add_message(_IngameMessage(point=point, id=None,
                                                msg_type=msg_type,
                                                display=visible_for,
                                                created=self.msgcount.next(),
                                                message=messagetext,
        ...
        for index, message in enumerate(self.active_messages):
            ...
            if message.x is not None and message.y is not None:
                ...
                callback = Callback.ChainedCallbacks(
                    callback,
                    # this makes it so the order
                    # of callback assignment doesn't matter
            ...
            if message.msg_type == "logbook":
                ...
                callback = Callback.ChainedCallbacks(
                    callback,
                    # this makes it so the order of
                    # callback assignment doesn't matter
    ...
    def show_text(self, index):
        ...
        try:
            ...
            self.log.error(u'Tried to access message at index %s,'
                           u' only have %s. Messages:',
                           index, len(self.active_messages))
            self.log.error(u'\n'.join(unicode(m) for m in
    ...
    def end(self):
        self.hide_text()
        self.widget.findChild(name="button_space").removeAllChildren()
        # Remove old buttons
    ...
    def save(self, db):
        for message in self.active_messages:
            if message.id is not None and message.id != 'CHAT':
                # only save default messages (for now)
                db("INSERT INTO message_widget_active (id, x, y, read, "
                   "created, display, message) "
                   "VALUES (?, ?, ?, ?, ?, ?, ?)",
        ...
        for message in self.archive:
            if message.id is not None and message.id != 'CHAT':
                db("INSERT INTO message_widget_archive (id, x, y, read, "
                   "created, display, message) VALUES (?, ?, ?, ?, ?, ?, ?)",
                   message.id, message.x, message.y, int(message.read),
                   message.created, message.display, message.message)
        for message in self.chat:
            # handle 'CHAT' special case: display is 0
            # (do not show old chat on load)
            db("INSERT INTO message_widget_archive (id, x, y, read, created, "
               "display, message) VALUES (?, ?, ?, ?, ?, ?, ?)",
               message.id, message.x, message.y, int(message.read),
               message.created, 0, message.message)
    ...
    def load(self, db):
        ...
        for (msg_id, x, y, read, created, display, message) in messages:
            msg = _IngameMessage(point=Point(x, y), id=msg_id, created=created,
                                 read=bool(read), display=bool(display),
                                 message=message)
        ...
        count = max([-1] + [m.created for m in self.active_messages +
                            self.archive + self.chat]) + 1
        self.msgcount = itertools.count(count)
        # start keyword only works with 2.7+
        self.draw_widget()


class _IngameMessage(object):
    """Represents a message that is to be displayed in the MessageWidget.
    The message is used as a string template,
    meaning it can contain placeholders
    ...
    @param point: Point position on the map
                  where the action took place, or None.
    @param id: message id string, needed to retrieve
               the message from the database.
    @param created: tickid when the message was created.
                    Keeps message order after load.

    @param msg_type: messages coupled with logbook entries use this to link
                     to pages
    @param read:  # TODO
    @param display:  # TODO
    @param message: message text to display. Loads preset for `id` if None.
    @param message_dict: dict with strings to replace in the message, e.g.
                         {'player': 'Arthus'}
    @param icon_id: which icon to display. Loads preset for `id` if None.
    """
    def __init__(self, point, id, created, msg_type=None, read=False,
                 display=None, message=None, message_dict=None, icon_id=None):
        ...
        self.display = display if display is not None else \
            horizons.globals.db.get_msg_visibility(id)
        icon = icon_id if icon_id is not None else \
            horizons.globals.db.get_msg_icon_id(id)
        self.path = horizons.globals.db.get_msg_icon_path(icon)
        if message is not None:
            assert isinstance(message,
                              unicode), "Message is not unicode: {0!s}".format(message)
            self.message = message
        else:
            msg = _(horizons.globals.db.get_msg_text(id))
            # TODO why can message_dict not be used with custom messages
            # (`if` branch above)
            try:
                self.message = msg.format(**message_dict if
                                          message_dict is not None else {})
            except KeyError as err:
                self.message = msg
                self.log.warning(u'Unsubstituted string %s in %s message "%s",'
                                 u' dict %s', err, msg, id, message_dict)

    def __repr__(self):
        return "{0: 4d}: {1!s} {2!s} {3!s}{4!s}".format(
    ...
    def __unicode__(self):
        return u"{0: 4d}: {1!s}  '{2!s}'  {3!s} {4!s}{5!s}".format(
            self.created, self.id, self.message,
            '({0!s},{1!s}) '.format(self.x, self.y) if self.x and self.y else '',
            'R' if self.read else ' ',
            'D' if self.display else ' ')
