...
class Inventory(Container):
    ...
    # uncached: required when resource icons should appear multiple times
    # at any given moment on the screen.
    # this is usually not the case with single inventories,
    # but e.g. for trading.
    # display_legend: whether to display a string explanation
    #                 about slot limits

    UNUSABLE_SLOT_IMAGE = "content/gui/icons/resources/none_gray.png"

    def __init__(self, uncached=False, display_legend=True, items_per_line=4,
                 **kwargs):
        ...
        # negative values are fine, 0 is not
    ...
    def init(self, db, inventory, ordinal=None):
        ...
        @param ordinal: {res: (min, max)} Display ordinal scale
                        with these boundaries instead of numbers
                        for a particular resource. Currently implemented
                        via ImageFillStatusButton.
        """
    ...
    def _draw(self, vbox, current_hbox, index=0):
        ...
        if isinstance(self._inventory, PositiveTotalNumSlotsStorage):
            # limited number of slots. We have to switch unused slots with
            # newly added ones on overflow

            while len(self._res_order) + len(
                    new_res) > self._inventory.slotnum:
        ...
        # add remaining slots for slotstorage or just add it without
        # consideration for other storage kinds
        ...
        for resid in self._res_order:
            ...
            if self.ordinal:
                ...
                amount = ""
                # do not display exact information for resource deposits
        ...
        if isinstance(self._inventory, TotalStorage):
            # if it's full, the additional slots have to be marked
            # as unusable (#1686)
            ...
            if not self._inventory.get_free_space_for(0):
                for i in xrange(index, self.items_per_line):
                    ...
                    # set min & max_size to prevent pychan to expand
                    # this dynamic widget (icon)
                    button.min_size = button.max_size = \
                        ImageFillStatusButton.ICON_SIZE
                    current_hbox.addChild(button)
