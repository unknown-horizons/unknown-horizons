...
class PlayersShips(StatsWidget):
	...
	def refresh(self):
		...
		for ship in sorted(self.session.world.ships, key = lambda ship: (ship.get_component(NamedComponent).name, ship.worldid)):
			if ship.owner is player and ship.has_component(SelectableComponent):
				...
				events['{0!s}/mouseClicked'.format(name_label.name)] = Callback(self._go_to_ship, ship)
				cb = Callback(self.session.ingame_gui.show_change_name_dialog, ship)
				events['{0!s}/mouseClicked'.format(rename_icon.name)] = cb
				events['{0!s}/mouseClicked'.format(status_label.name)] = Callback(self._go_to_point, status_position)
	...
	def _add_line_to_gui(self, ship, sequence_number):
		sequence_number_label = Label(name='sequence_number_{0:d}'.format(ship.worldid))
		sequence_number_label.text = unicode(sequence_number)
		sequence_number_label.min_size = sequence_number_label.max_size = (15, 20)

		ship_name = Label(name='ship_name_{0:d}'.format(ship.worldid))
		...
		rename_icon = RenameImageButton(name='rename_{0:d}'.format(ship.worldid))
		...
		ship_type = Label(name='ship_type_{0:d}'.format(ship.worldid))
		ship_type.text = ship.classname
		ship_type.min_size = ship_type.max_size = (60, 20)

		weapons = Label(name='weapons_{0:d}'.format(ship.worldid))
		if isinstance(ship, FightingShip):
			weapon_list = []
			for weapon_id, amount in sorted(ship.get_weapon_storage().itercontents()):
				weapon_list.append('{0:d} {1!s}'.format(amount, self.session.db.get_res_name(weapon_id)))
			if weapon_list:
		...
		health = Label(name='health_{0:d}'.format(ship.worldid))
		health_component = ship.get_component(HealthComponent)
		health.text = u'{0:d}/{1:d}'.format(health_component.health, health_component.max_health)
		health.min_size = health.max_size = (65, 20)

		status = Label(name='status_{0:d}'.format(ship.worldid))
